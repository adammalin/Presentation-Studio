...
function sourceComparisonList(label, values) {
  if (!Array.isArray(values) || values.length === 0) return "";
  return `<div><b>${escapeHtml(label)}</b><ul>${values.map((value) => `<li>${escapeHtml(value)}</li>`).join("")}</ul></div>`;
}

function sourceComparison(review) {
  const comparison = review?.sourceComparison;
  if (!comparison) return "";
  const preference = comparison.preferred === "source"
    ? "Source wins — candidate must be revised or reverted"
    : comparison.preferred === "candidate"
      ? "Candidate wins"
      : "Visually equivalent";
  return `<section class="source-comparison ${escapeHtml(comparison.preferred)}"><strong>${escapeHtml(preference)}</strong><div class="source-comparison-grid">${sourceComparisonList("Source strengths", comparison.sourceStrengths)}${sourceComparisonList("Candidate improvements", comparison.candidateImprovements)}${sourceComparisonList("Candidate regressions", comparison.candidateRegressions)}</div></section>`;
}
...
function qualificationReportHtml(report) {
  ...
  const slides = report.slides.map((slide) => `<article id="slide-${slide.slideNumber}">
    ...
    ${slide.review ? `<section class="review ${slide.review.recordedVerdict}"><strong>Visual pass ${slide.review.pass} · ${escapeHtml(slide.review.recordedVerdict)}</strong><span>${escapeHtml(slide.review.rationale)}</span><small>${escapeHtml(slide.review.reviewer)} · ${escapeHtml(slide.review.reviewedAt)}</small></section>${sourceComparison(slide.review)}` : ""}
