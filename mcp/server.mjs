...
server.registerTool("get_studio_web_scene", {
  ...
  description: "Read the canonical semantic HTML/CSS scene at 13.333 × 7.5 inches for one imported PowerPoint slide: complete source text, exact locked node text and table cells, paragraph-level semantic atom candidates, content-mapping coverage, source-bound editable node IDs, source and current frames in inches, repeated-component group roles, media bindings, ORNL design tokens, separate source-slide dimensions, and the recommended shared layout recipe. If coverage is incomplete, grouped or unsupported text remains protected and fresh composition is held instead of silently omitting it. This is the AI and human design surface—use it to make an actual composition decision instead of merely shrinking fonts or preserving weak source coordinates. The source PowerPoint remains the preservation envelope and its native render remains final visual authority.",
  inputSchema: { deckId: z.string().min(1).max(120), slideNumber: z.number().int().min(1) },
  annotations: { readOnlyHint: true, destructiveHint: false, openWorldHint: false },
