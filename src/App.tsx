...
import { buildAgentRunbook } from "./lib/agent-runbook";
...
import { isolateNativePowerPointObjects, isolateNativePowerPointSlide, nativeIsolationShapeIds } from "./lib/native-object-isolation";
import { bindNativeMeasurement, compareNativeMeasurementPackets, remapSingleSlideNativeMeasurement, type NativeMeasurementPacket } from "./lib/native-measurement";
...
function mcpPhaseForOperation(operation: string, input: Record<string, unknown>): McpActivityPhase {
  ...
  if (operation.startsWith("stage_") || operation.startsWith("solve_") || operation === "set_deck_template_target" || operation === "create_studio_presentation" || operation === "fit_scene_to_layout" || operation === "refine_studio_layout" || operation === "repair_studio_objective_issues" || operation === "reconstruct_studio_concept" || operation === "publish_studio_component_style" || operation === "publish_studio_table_exemplar" || operation === "plan_studio_table_continuation") return "fixing";
  return "working";
}
...
function deckAuditNeedsUpgrade(deck: DeckJob): boolean {
  return Boolean(deck.audit && (
    sceneNeedsRebuild(deck)
    || deck.audit.semanticVisualVersion !== PPTX_AUDIT_SEMANTIC_VISUAL_VERSION
    || !deck.audit.slideSize
    || !Array.isArray(deck.audit.editableObjects)
    || (deck.audit.slideCount > 0 && deck.audit.editableObjects.length === 0)
    || new Set(deck.audit.editableObjects.map((object) => object.id)).size !== deck.audit.editableObjects.length
    || deck.audit.slides.some((slide) => !slide.sourcePartSha256)
    || deck.audit.textBoxes.some((textBox) => typeof textBox.text !== "string" || (textBox.characterCount > 0 && textBox.text.length === 0) || (textBox.characterCount > 0 && textBox.estimatedOpticalLeftEmu <= 0))
  ));
}

async function upgradeOpenedProjectAudits(opened: PresentationStudioProject): Promise<PresentationStudioProject> {
  const decks: DeckJob[] = [];
  for (const deck of opened.decks) {
    if (!deckAuditNeedsUpgrade(deck)) { decks.push(deck); continue; }
    const source = sourceForDeck(opened, deck);
    if (!source?.bytes) throw new Error(`The embedded source for ${deck.name} is unavailable, so its saved audit cannot be upgraded safely.`);
    const audit = await auditPptx(bytesFrom(source.bytes));
    decks.push(withCompiledScene(deckWithAutomaticTemplateRouting({ deck, audit })));
  }
  return decks.some((deck, index) => deck !== opened.decks[index]) ? { ...opened, decks } : opened;
}
    ...
    <div className="view-stack">
      ...
      <section className="classification-card">
        ...
        <div className="template-actions">
          <label htmlFor="target-template">Design target · ORNL by default</label>
          ...
          {deck.targetTemplateConfirmedAt && <span className="confirmation"><Check size={14} />{deck.targetTemplateDecisionSource === "automatic-default" ? "ORNL default adopted" : ["sponsor-source", "custom-source"].includes(deck.targetTemplateId ?? "") ? "Source preservation selected by user" : "ORNL selected by user"}</span>}
        </div>
      </section>
      ...
      <div className="split-grid">
        ...
        <section className="panel">
          ...
          <dl className="check-list">
            ...
            <div><dt>Hyperlinks</dt><dd className="good">{audit.externalHyperlinkCount ? `${audit.externalHyperlinkCount} preserved` : "None"}</dd></div>
            <div><dt>Other external links</dt><dd className={audit.containsBlockingExternalRelationships ? "bad" : "good"}>{audit.containsBlockingExternalRelationships ? `${audit.blockingExternalRelationshipCount} · Review` : "None"}</dd></div>
...
function SlidesView({ deck, catalog, nativeRender, outputSlides, loading, revision, threads, openRequest, deckBuildReady, resourceCount, onAddDeck, onOpenResources, onSaveThread, onDeleteThread, onStageGeometry, onOpenStudioSlide, onSaveDeck }: { deck?: DeckJob; catalog?: SlideRenderCatalog; nativeRender?: NativeRenderResult; outputSlides?: StudioCompositionExportResult["outputSlides"]; loading: boolean; revision: string; threads: DesignThread[]; openRequest?: SlideWorkspaceRequest; deckBuildReady: boolean; resourceCount: number; onAddDeck: () => void; onOpenResources: () => void; onSaveThread: (slide: SlideRenderPreview, anchor: DesignThread["anchor"], comment: string, submit: boolean, outputSlideNumber?: number) => void; onDeleteThread: (threadId: string) => void; onStageGeometry: (object: SlideEditableObject, target: { x: number; y: number; width: number; height: number }, rationale: string) => void; onOpenStudioSlide: (slideNumber: number) => void; onSaveDeck: () => void }) {
  ...
  return <div className="view-stack slides-view">
    ...
    {selected && catalog && <section className="slide-review panel">
      ...
      <div className="slide-review-body">
        <div className="slide-review-stage">
          ...
          <div className={`slide-representation-note ${selectedNativeReady ? "native" : "fallback"}`}><ShieldCheck size={15} /><span><strong>{centralDesignActive ? selectedNativeReady ? "Central Studio design · PowerPoint native" : "Central Studio design · compiling" : editMode ? "Non-destructive object editor" : nativeReady ? "PowerPoint-native representation" : "Approximate OOXML fallback"}</strong> {centralDesignActive ? selectedNativeReady ? "This is the exact editable PowerPoint projection of the saved Studio scene. Comments and layout edits target this revision." : "The JSON/HTML scene remains the source of truth while Presentation Studio automatically regenerates and checks its editable PowerPoint result." : editMode ? "Drag a highlighted object, use measured controls, then stage it into Current/Proposal review. Imported slide bytes stay untouched." : nativeReady ? "These pixels came from Microsoft PowerPoint. Structured editable-object overlays remain bound to the same source revision." : nativeRender?.warnings[0] ?? "This diagnostic reconstruction is not authoritative for wrapping, masters, tables, equations, or crop behavior."}</span></div>
        </div>
    ...
    {!selected && <section className="current-slide-gallery panel"><div className="panel-heading"><div><h2>{centralDesignActive ? "Central converted design" : "Slides"}</h2><p>{centralDesignActive ? "Every thumbnail is the exact PowerPoint projection of the saved Studio JSON/HTML scene. Choose a slide to review, comment, or edit its shared design." : `Select any ${proposalWorkspace ? "proposed" : "current"} design for a closer, zoomable review and location-anchored comments.`}</p></div><span className="quiet-label">{proposalWorkspace ? "Proposal" : centralDesignActive ? "One presentation" : "Current"} · {PRESENTATION_DESIGN_STANDARD.defaults.slide.aspectRatio} target</span></div>
      {loading && <div className="slide-gallery-loading"><ArrowsClockwise className="spinner" size={25} /><strong>{centralDesignActive ? "Compiling the saved Studio scene" : "Building local slide previews"}</strong><span>{centralDesignActive ? "Generating editable PowerPoint from the central design, then checking its exact PowerPoint pixels and text geometry." : "Reading the embedded source, master, layouts, media, and native table structure."}</span></div>}
...
function SlideDesignCanvas({ nativeRender, slideNumber, catalog, layout, label, requireNative = false }: { nativeRender?: NativeRenderResult; slideNumber: number; catalog?: PreviewCanvasCatalog; layout?: TemplateLayoutPreview; label: string; requireNative?: boolean }) {
  ...
  if (requireNative) return <span className="proposal-preview-wait"><Monitor size={22} />Compiling this saved Studio design into its exact PowerPoint result…</span>;
...
function StudioView({ deck, catalog, nativeRender, freshPreviews, centralBuild, templateCatalog, templateNativeRender, resources, requestedSlideNumber, deckBuildReady, qualification, canUndo, canRedo, onUndo, onRedo, onInitialize, onRecompose, onMoveNode, onMoveNodes, onStyleNode, onPublishComponent, onArrangeSelection, onRepairObjectiveIssues, onUpdateTableDesign, onResizeTableColumn, onResizeTableRow, onUpdateTableCell, onPublishTableExemplar, onApplyTableExemplar, onPlanTableContinuation, onClearTableContinuation, onUpdateConnector, onUpdateFigure, onCreateVisualNeed, onHoldVisualNeed, onAttachConcept, onDetachConcept, onReconstructConcept, onPreviewFresh, onPreviewAll, onQualify, onRevealQualification, onSaveFresh, onSaveDeck }: { deck?: DeckJob; catalog?: SlideRenderCatalog; nativeRender?: NativeRenderResult; freshPreviews?: Record<string, StudioFreshPreview>; centralBuild?: StudioDeckBuild; templateCatalog?: TemplateCatalog; templateNativeRender?: NativeRenderResult; resources: ProjectResource[]; requestedSlideNumber?: number; deckBuildReady: boolean; qualification?: StudioDeckQualification; canUndo: boolean; canRedo: boolean; onUndo: () => void; onRedo: () => void; onInitialize: () => void; onRecompose: (slideNumber: number, recipe: StudioLayoutRecipe, layoutId?: string) => void; onMoveNode: (slideNumber: number, nodeId: string, frame: StudioWebFrame) => void; onMoveNodes: (slideNumber: number, updates: Array<{ nodeId: string; frame: StudioWebFrame }>) => void; onStyleNode: (slideNumber: number, nodeId: string, patch: Partial<Pick<StudioWebNode["style"], "fontSizePt" | "fontWeight" | "color" | "textAlign" | "verticalAlign" | "objectFit">>) => void; onPublishComponent: (slideNumber: number, nodeId: string) => void; onArrangeSelection: (slideNumber: number, nodeIds: string[], mode: StudioConstraintRequest["mode"]) => void; onRepairObjectiveIssues: (slideNumber: number) => void; onUpdateTableDesign: (slideNumber: number, nodeId: string, patch: StudioTableDesignPatch) => void; onResizeTableColumn: (slideNumber: number, nodeId: string, column: number, widthInches: number) => void; onResizeTableRow: (slideNumber: number, nodeId: string, row: number, heightInches: number) => void; onUpdateTableCell: (slideNumber: number, nodeId: string, cellId: string, patch: StudioTableCellDesignPatch) => void; onPublishTableExemplar: (slideNumber: number, nodeId: string) => void; onApplyTableExemplar: (definitionId: string) => void; onPlanTableContinuation: (slideNumber: number, nodeId: string, maximumBodyRowsPerSlide: number) => void; onClearTableContinuation: (slideNumber: number, nodeId: string) => void; onUpdateConnector: (slideNumber: number, nodeId: string, design: StudioConnectorDesign) => void; onUpdateFigure: (slideNumber: number, treatment: StudioFigureTreatment) => void; onCreateVisualNeed: (slideNumber: number, type: StudioVisualNeed["type"]) => void; onHoldVisualNeed: (slideNumber: number, visualNeedId: string) => void; onAttachConcept: (slideNumber: number, visualNeedId: string, resourceId: string) => void; onDetachConcept: (slideNumber: number, referenceId: string) => void; onReconstructConcept: (slideNumber: number, referenceId: string) => void; onPreviewFresh: (slideNumber: number) => void; onPreviewAll: () => void; onQualify: () => void; onRevealQualification: () => void; onSaveFresh: (slideNumber: number) => void; onSaveDeck: () => void }) {
  ...
  const freshCandidateAccepted = Boolean(freshNativeSlide && slide.qualityReview?.recordedVerdict === "ready" && slide.qualityReview.slideUpdatedAt === slide.updatedAt && slide.qualityReview.rasterSha256 === freshNativeSlide.sha256);
  const centralCandidateAccepted = Boolean(currentCentralBuild && qualification?.sceneRevision === scene.revision && qualification.report.status === "review-complete");
  const exportResultSource = needsFreshExport ? centralCandidateAccepted ? centralNativeSource : freshCandidateAccepted ? freshNativeSource : nativeSlideSource : nativeSlideSource;
  ...
  const resultStatus = (item: StudioWebScene["slides"][number]) => {
    ...
    if (centralCandidateAccepted && currentCentralBuild?.outputSlides.some((output) => output.sourceSlideNumber === item.slideNumber) && currentCentralBuild.nativeRender.status === "ready") return "ready";
    const preview = freshPreviews?.[`${deck.id}:${item.slideNumber}`];
    const rendered = preview?.slideUpdatedAt === item.updatedAt && preview.nativeRender?.status === "ready" && preview.nativeMeasurement?.status === "ready" ? preview.nativeRender.slides[0] : undefined;
    return rendered && item.qualityReview?.recordedVerdict === "ready" && item.qualityReview.slideUpdatedAt === item.updatedAt && item.qualityReview.rasterSha256 === rendered.sha256 ? "ready" : "pending";
  };
  ...
  const deckExportReady = deckBuildReady && centralCandidateAccepted;
  ...
  return <div className="view-stack studio-view">
    <header className="view-header compact">
      ...
      <div className="header-actions studio-header-actions">
        ...
        <button className="button primary small" disabled={!deckExportReady} title={deckExportReady ? "Save the visually accepted central design as one editable PowerPoint presentation." : deckBuildReady ? "Complete the PowerPoint-native visual review before export." : "Convert and build every slide before exporting one central presentation."} onClick={onSaveDeck}><FileArrowDown size={16} />Export presentation</button>
        ...
        {currentFreshPreview && <button className="button secondary small" disabled={!freshCandidateAccepted || currentFreshPreview.nativeMeasurement?.status !== "ready" || currentFreshPreview.nativeMeasurement.authority !== "powerpoint-native"} title={freshCandidateAccepted ? "Save the exact visually accepted slide result." : "Record a ready visual verdict for this exact PowerPoint raster before saving."} onClick={() => onSaveFresh(slide.slideNumber)}><FileArrowDown size={16} />Save this result</button>}
      </div>
    </header>
...
function DesignsView({ catalog, installedAt, loading, nativeRender, nativeLoading, onInstall }: { catalog?: TemplateCatalog; installedAt?: string; loading: boolean; nativeRender?: NativeRenderResult; nativeLoading: boolean; onInstall: () => void }) {
  ...
  return <div className="view-stack designs-view">
    ...
    <section className="design-feature">
      ...
      <div className="design-feature-copy"><span className="field-label">Selected design</span><h2>{selected.name}</h2><p>{nativeRender?.status === "ready" ? "The base artwork below is an authoritative Microsoft PowerPoint render of the installed layout. Studio overlays its semantic editable regions for human and AI composition." : "This local structural view combines the actual template geometry, artwork, and semantic editable regions while the PowerPoint-native preview is unavailable."}</p><dl><div><dt>Layout contract</dt><dd>{selected.semantic ? `${selected.semantic.contract.family.replaceAll("-", " ")} · ${selected.semantic.contract.selectionPolicy.replaceAll("-", " ")} · ${selected.semantic.contract.surface} surface` : "Template-defined"}</dd></div><div><dt>Communication jobs</dt><dd>{selected.semantic?.contract.compatibleArchetypes.length ? selected.semantic.contract.compatibleArchetypes.map((archetype) => archetype.replaceAll("-", " ")).join(" · ") : "Special-purpose native artwork"}</dd></div><div><dt>Best fit</dt><dd>{selected.semantic?.summary ?? (selected.placeholderTypes.length ? selected.placeholderTypes.join(" · ") : "Freeform / template furniture")}</dd></div><div><dt>Content constraints</dt><dd>{selected.semantic ? `${selected.semantic.constraints.maxTitleLines} title lines max · ${selected.semantic.capabilities.bodySlots ? `${selected.semantic.constraints.minimumBodyFontPt} pt body minimum` : `${selected.semantic.constraints.minimumCaptionFontPt} pt label minimum`} · ${selected.semantic.constraints.preferredBodyDensity} density` : "Template-defined"}</dd></div><div><dt>PowerPoint structure</dt><dd>Master, custom layout, theme, inherited artwork, and editable slots preserved</dd></div></dl><div className="inline-note design-note"><Info size={16} />Green dashed regions are Studio guides, not template artwork. Selecting a design changes nothing; AI and new-deck routing use these same contracts and constraints.</div></div>
    </section>
...
function RulesView({ deck, exemplarCount }: { deck?: DeckJob; exemplarCount: number }) {
  ...
  return <div className="view-stack"><header className="view-header compact"><div><p className="eyebrow">Presentation Design Standard</p><h1>Fast defaults, deterministic exceptions</h1><p>One versioned ruleset drives the app, preflight, cleanup engine, reports, tests, and MCP contract.</p></div><span className="standard-version">v{standard.version}</span></header>
    ...
    <div className="rule-grid"><RuleCard title="Legacy font normalization" scope="Font family only" status={ornlReady ? "Ready" : "Waiting for template"} detail="Maps direct Century Gothic and Arial slide markup to Aptos. Symbol fonts, theme tokens, text strings, object identity, and geometry remain untouched." /><RuleCard title="Exact-content guard" scope="Every exported slide" status="Always on" detail="Compares the visible-text hash and slide count before and after cleanup. The export is rejected if either changes." /><RuleCard title="Advanced content hold" scope="Deck safety" status="Always on" detail="Preserves ordinary hyperlinks while blocking macros, embedded OLE, linked media/data, and unknown external relationships for human review." /><RuleCard title="Approved table exemplar" scope="Tables only" status={exemplarCount > 0 ? `${exemplarCount} registered` : "Product fallback active"} detail="A registered exemplar overrides compatible table tokens without copying cell content. Otherwise the versioned minimal-rule profile is used." /></div></div>;
}
...
export default function App() {
  ...
  const checkpointSaverRef = useRef<LatestOnlySaver<AutosaveSnapshot> | undefined>(undefined);
  const packageSaverRef = useRef<LatestOnlySaver<AutosaveSnapshot> | undefined>(undefined);
  const fullPackageInventoryRef = useRef<string | undefined>(undefined);
  const auditGeometryUpgradeAttempted = useRef(new Set<string>());
  const automaticStudioBuildAttempted = useRef(new Set<string>());
  ...
  useEffect(() => { autosaveUiRef.current = autosaveUi; }, [autosaveUi]);
  useEffect(() => { studioEditHistoryRef.current = {}; setStudioHistoryVersion((value) => value + 1); }, [project.project.id]);
  useEffect(() => {
    if (!desktop) return;
    const checkpointSaver = createLatestOnlySaver<AutosaveSnapshot>({
      delayMs: 80,
      revision: (snapshot) => snapshot.project.project.updatedAt,
      save: async (snapshot) => {
        let bytes = buildProjectRecoveryCheckpoint(snapshot.project);
        if (snapshot.password) bytes = await encryptProjectPackage(bytes, snapshot.password);
        await desktop.checkpointProjectState({ bytes, encrypted: Boolean(snapshot.password) });
      },
      onProgress: (checkpoint) => setAutosaveUi((current) => ({ ...current, checkpoint })),
    });
    const packageSaver = createLatestOnlySaver<AutosaveSnapshot>({
      delayMs: 900,
      revision: (snapshot) => snapshot.project.project.updatedAt,
      save: async (snapshot) => {
        let bytes = await buildProjectPackage(snapshot.project);
        if (snapshot.password) bytes = await encryptProjectPackage(bytes, snapshot.password);
        await desktop.autosaveProject({ bytes, encrypted: Boolean(snapshot.password) });
      },
      onProgress: (packageProgress) => setAutosaveUi((current) => ({ ...current, package: packageProgress })),
    });
    checkpointSaverRef.current = checkpointSaver;
    packageSaverRef.current = packageSaver;
    return () => {
      checkpointSaverRef.current = undefined;
      packageSaverRef.current = undefined;
      void checkpointSaver.dispose().catch(() => undefined);
      void packageSaver.dispose().catch(() => undefined);
    };
  }, [desktop]);
  ...
  useEffect(() => {
    ...
    const candidates = project.decks.filter((deck) => deckAuditNeedsUpgrade(deck) && !auditGeometryUpgradeAttempted.current.has(auditUpgradeKey(deck)));
    ...
    for (const deck of candidates) {
      ...
      void auditPptx(source.bytes).then((audit) => {
        ...
        if (!canceled) setError(caught instanceof Error ? `Presentation Studio could not upgrade the saved audit for ${deck.name}: ${caught.message}` : `Presentation Studio could not upgrade the saved audit for ${deck.name}.`);
      });
    }
    return () => { canceled = true; };
  ...
  const getOrBuildNativeSlideMeasurement = useCallback(async (deck: DeckJob, slideNumber: number, representation: "current" | "proposal" = "current", current = projectRef.current): Promise<NativeMeasurementPacket> => {
    if (!desktop) return bindNativeMeasurement(deck, undefined, { slideNumbers: [slideNumber] });
    const proposalRepresentation = representation === "proposal";
    const selectedSignature = proposalRepresentation && deck.proposal ? deck.proposal.changes.filter((change) => change.selected).map((change) => change.id).sort().join("|") : "source";
    const cacheKey = `${deck.id}:${representation}:${deck.sourceSha256}:${deck.proposal?.id ?? "none"}:${selectedSignature}:slide-${slideNumber}`;
    const cached = nativeMeasurementsRef.current.get(cacheKey);
    if (cached) return bindNativeMeasurement(deck, cached, { slideNumbers: [slideNumber] });
    const source = sourceForDeck(current, deck);
    if (!source?.bytes) throw new Error("The embedded PowerPoint source is unavailable for native slide measurement.");
    let bytes = bytesFrom(source.bytes);
    let name = deck.name;
    if (proposalRepresentation) {
      if (!deck.proposal) throw new Error("Stage a proposal before requesting proposal measurements.");
      const materialized = await buildCleanupProposalPptx(bytes, deck.proposal, { templateBytes: templateSourceBytes });
      bytes = materialized.bytes;
      name = `${cleanFileStem(deck.name)}_proposal.pptx`;
    }
    const isolated = await isolateNativePowerPointSlide({ sourceBytes: bytes, slideNumber });
    const measured = await desktop.measurePowerPoint({ name: `${cleanFileStem(name)}_slide-${slideNumber}.pptx`, bytes: isolated.bytes });
    const qualified = measured.status === "ready" && measured.slideCount !== 1
      ? { ...measured, status: "failed" as const, authority: "unknown" as const, slides: [], warnings: [`PowerPoint returned measurements for ${measured.slideCount} slides from an isolated one-slide inspection package.`] }
      : remapSingleSlideNativeMeasurement(measured, slideNumber);
    if (qualified.status === "ready") {
      nativeMeasurementsRef.current.set(cacheKey, qualified);
      while (nativeMeasurementsRef.current.size > 24) nativeMeasurementsRef.current.delete(nativeMeasurementsRef.current.keys().next().value as string);
    }
    return bindNativeMeasurement(deck, qualified, { slideNumbers: [slideNumber] });
  }, [desktop, templateSourceBytes]);
  ...
  const getOrBuildInspectionSlideRender = useCallback(async (deck: DeckJob, slideNumber: number, representation: "current" | "proposal" = "current", current = projectRef.current) => {
    if (!desktop) throw new Error("PowerPoint-native inspection requires the desktop app.");
    const proposalRepresentation = representation === "proposal";
    const selectedSignature = proposalRepresentation && deck.proposal ? deck.proposal.changes.filter((change) => change.selected).map((change) => change.id).sort().join("|") : "source";
    const cacheKey = `${deck.id}:${representation}:${deck.sourceSha256}:${deck.proposal?.id ?? "none"}:${selectedSignature}:slide-${slideNumber}:png-2200`;
    const cached = inspectionRendersRef.current.get(cacheKey);
    if (cached) return cached;
    const source = sourceForDeck(current, deck);
    if (!source?.bytes) throw new Error("The embedded PowerPoint source is unavailable for inspection rendering.");
    ...
    const isolated = await isolateNativePowerPointSlide({ sourceBytes: bytes, slideNumber });
    const render = await desktop.renderPowerPoint({ name: `${cleanFileStem(name)}_slide-${slideNumber}.pptx`, bytes: isolated.bytes, width: 2200, format: "png" });
    if (render.status !== "ready" || render.slideCount !== 1 || render.slides.length !== 1) throw new Error(render.warnings[0] ?? "PowerPoint-native one-slide inspection rendering failed qualification.");
    const remapped = { ...render, slides: [{ ...render.slides[0], number: slideNumber }] };
    inspectionRendersRef.current.set(cacheKey, remapped);
    while (inspectionRendersRef.current.size > 24) inspectionRendersRef.current.delete(inspectionRendersRef.current.keys().next().value as string);
    return remapped;
  }, [desktop, templateSourceBytes]);
  ...
  useEffect(() => {
    if (!desktop || !project.settings.autosave || !projectHasRecoverableWork(project)) return;
    const snapshot = { project, password: secureAutosavePassword };
    const inventoryKey = projectResourceInventoryKey(project);
    const resourceInventoryChanged = fullPackageInventoryRef.current !== inventoryKey;
    fullPackageInventoryRef.current = inventoryKey;
    checkpointSaverRef.current?.request(snapshot);
    packageSaverRef.current?.request(snapshot);
    if (resourceInventoryChanged) void packageSaverRef.current?.flush().catch(() => undefined);
  ...
  useEffect(() => {
    ...
    return desktop.onMcpCommand(async (request) => {
      ...
      const commandStartProject = projectRef.current;
      const commandStartRevision = commandStartProject.project.updatedAt;
      ...
      if (request.operation === "get_agent_runbook") {
        const requestedDeckId = typeof request.input.deckId === "string" ? request.input.deckId : selectedDeckId;
        const deck = current.decks.find((item) => item.id === requestedDeckId) ?? current.decks[0];
        if (!deck?.audit) throw new Error("Open and audit a presentation before requesting the agent runbook.");
        const build = studioDeckBuildsRef.current[deck.id];
        const qualification = studioDeckQualificationsRef.current[deck.id]?.report;
        return buildAgentRunbook({
          deck,
          projectUpdatedAt: current.project.updatedAt,
          templateInstalled: Boolean(templateCatalog),
          buildSceneRevision: build?.sceneRevision,
          qualification,
          threads: current.designThreads,
        });
      }
      ...
      if (request.operation === "recommend_slide_layouts") {
        ...
        if (deckTemplateWorkflow(deck) === "source-template-cleanup") throw new Error("This deck is preserving a detected sponsor/custom template. ORNL Template Pack layout recommendations are unavailable unless the user explicitly chooses ORNL cross-template conversion.");
      ...
      if (request.operation === "get_studio_composition_plan") {
        if (!templateCatalog) throw new Error("Install an authorized PowerPoint Template Pack before requesting a Studio composition plan.");
        const deck = current.decks.find((item) => item.id === request.input.deckId);
        ...
        if (deckTemplateWorkflow(deck) === "source-template-cleanup") throw new Error("This deck is preserving a detected sponsor/custom template. ORNL Studio composition is unavailable unless the user explicitly chooses ORNL cross-template conversion.");
        const slideNumber = Number(request.input.slideNumber);
        if (!Number.isInteger(slideNumber) || slideNumber < 1 || slideNumber > deck.audit.slideCount) throw new Error(`Choose a slide from 1 to ${deck.audit.slideCount}.`);
        const rawArchetype = typeof request.input.archetype === "string" ? request.input.archetype : undefined;
        if (rawArchetype && !STUDIO_DESIGN_ARCHETYPES.includes(rawArchetype as StudioDesignArchetype)) throw new Error("Choose a supported Studio communication archetype.");
        ...
        if (!studioSlide) throw new Error("The requested slide is unavailable in the central Studio scene.");
        const auditedSlide = deck.audit.slides.find((slide) => slide.number === slideNumber);
        const profile = contentProfileForSlide(deck, slideNumber);
        const plan = planStudioComposition(profile, templateCatalog.layouts, {
          ...
          protectedSourceComposition: isProtectedOrnlTemplateSlide(deck, slideNumber) && !isUnqualifiedConvertedOrnlTitle(deck, slideNumber),
        ...
        return {
          ...
          profile,
          structure: {
            textNodes: studioSlide.nodes.filter((node) => node.kind === "text").length,
            imageNodes: studioSlide.nodes.filter((node) => node.kind === "image").length,
            tableNodes: studioSlide.nodes.filter((node) => node.kind === "table").length,
            connectorNodes: studioSlide.nodes.filter((node) => node.kind === "connector").length,
            nativeObjectNodes: studioSlide.nodes.filter((node) => node.kind === "native-object").length,
            repeatedImageSeries: Boolean(inferRepeatedImageSeries(studioSlide)),
          },
          plan,
          intervention,
      ...
      if (request.operation === "get_slide_design_work_order") {
        ...
        const slideNumber = Number(request.input.slideNumber);
        const [nativeRender, measurement] = await Promise.all([getOrBuildInspectionSlideRender(deck, slideNumber, "current", current), getOrBuildNativeSlideMeasurement(deck, slideNumber, "current", current)]);
      ...
      if (request.operation === "get_slide_inspection_packet") {
        ...
        if (!Number.isInteger(slideNumber) || slideNumber < 1 || slideNumber > deck.audit.slideCount) throw new Error(`Choose a slide from 1 to ${deck.audit.slideCount}.`);
        const [nativeRender, measurement] = await Promise.all([getOrBuildInspectionSlideRender(deck, slideNumber, representation, current), getOrBuildNativeSlideMeasurement(deck, slideNumber, representation, current)]);
        const baselineMeasurement = representation === "proposal" ? await getOrBuildNativeSlideMeasurement(deck, slideNumber, "current", current) : undefined;
        const workOrder = buildSlideDesignWorkOrder({ deck, slideNumber, projectUpdatedAt: current.project.updatedAt, templateCatalog, currentRender: nativeRender, currentMeasurement: measurement, threads: current.designThreads });
      ...
      if (request.operation === "get_deck_design_work_order") {
        ...
        const deck = current.decks.find((item) => item.id === request.input.deckId);
        if (!deck?.audit || !deck.scene) throw new Error("The requested deck does not have a current audit and hybrid scene.");
        // A deck work order is a routing summary, not a reason to measure all
        // 200 slides before the agent can begin. Large decks use the bounded
        // OOXML packet here and acquire authoritative PowerPoint geometry one
        // selected slide at a time through the inspection tools below.
        const [nativeRender, measurement] = await Promise.all([
          getOrBuildNativeRender(deck, "current", current),
          deck.audit.slideCount > 20 ? Promise.resolve(bindNativeMeasurement(deck)) : getOrBuildNativeMeasurement(deck, "current", current),
        ]);
      ...
      if (request.operation === "set_deck_template_target") {
        if (request.input.expectedUpdatedAt !== current.project.updatedAt) throw new Error("The project changed. Read list_decks again before changing the design target.");
        const deck = current.decks.find((item) => item.id === request.input.deckId);
        if (!deck?.audit) throw new Error("The requested deck is not open or audited.");
        const now = new Date().toISOString();
        const useOrnl = request.input.target === "ornl-default";
        const targetTemplateId = useOrnl ? PRESENTATION_DESIGN_STANDARD.defaults.template.id : deck.templateClassification === "sponsor" ? "sponsor-source" : "custom-source";
        const targeted = withCompiledScene({
          ...deck,
          operationScope: "reflow",
          targetTemplateId,
          targetTemplateConfirmedAt: now,
          targetTemplateDecisionSource: "user-selected",
          designProfile: useOrnl ? createOrnlDesignProfile("user-selected", now) : undefined,
          status: useOrnl ? "ready-for-cleanup" : "audited",
          studioScene: undefined,
          proposal: undefined,
        });
        clearDeckRenderState(deck.id);
        const next = touchProject({ ...current, decks: current.decks.map((item) => item.id === deck.id ? targeted : item) }, "mcp-template-target-set", `${useOrnl ? "Selected the current ORNL Template Pack and Studio design system" : "Explicitly preserved the detected source template"} for ${deck.name}; stale proposals and alternate Studio compositions were cleared.`);
        projectRef.current = next;
        setProject(next);
        setSelectedDeckId(deck.id);
        setActiveView(useOrnl ? "studio" : "slides");
        return {
          projectUpdatedAt: next.project.updatedAt,
          deck: { id: targeted.id, name: targeted.name, sourceTemplateClassification: targeted.templateClassification, targetTemplateId: targeted.targetTemplateId, targetTemplateDecisionSource: targeted.targetTemplateDecisionSource, templateWorkflow: deckTemplateWorkflow(targeted) },
          sourcePowerPointChanged: false,
          staleProposalCleared: true,
          staleStudioSceneCleared: true,
          instruction: useOrnl
            ? "Create the central Studio Web Scene, choose approved ORNL layouts or shared recipes for every non-title slide, use an approved sacred ORNL title layout for slide 1, then build and inspect PowerPoint-native results."
            : "Preserve the source master, layouts, artwork, and type system. Use only source-bound cleanup operations and PowerPoint-native Current/Proposal QA until the user selects ORNL again.",
        };
      }
      ...
      if (request.operation === "get_studio_web_scene") {
        ...
        return {
          ...
          instruction: isProtectedOrnlTemplateSlide(deck, slideNumber)
            ? isSacredOrnlTitleSlide(deck, slideNumber)
              ? `HARD RULE: the existing approved ORNL template composition on slide ${slideNumber} is sacred and source-preserved. Preserve its approved marks, artwork, photography, legal copy, geometry, master, and layout exactly. Do not send nodeFrames, nodeStyles, figure treatments, or another recipe for this slide.`
              : `HARD RULE: the approved converted ORNL template composition on slide ${slideNumber} is now sacred. Keep its exact template-layout recipe, artwork, editable content bindings, typography, and geometry unchanged. Do not send nodeFrames, nodeStyles, figure treatments, or another recipe for this slide.`
      ...
      if (request.operation === "get_studio_slide_critique") {
        ...
        return {
          ...
          intervention: resolveStudioIntervention(deck, slideNumber, studioSlide, studioSlide.designArchetype),
          sourceWinsGate: "Compare source strengths, candidate improvements, and candidate regressions. If the candidate is not clearly stronger within the selected intervention level, prefer the source and revise or revert.",
          ...
          instruction: critique.verdict === "ready" ? `Inspect the original and export images${conceptImages.length ? " plus each concept-only image for only its approved visual influences" : ""} for issues the deterministic critic cannot see. Explicitly inventory source strengths, candidate improvements, and candidate regressions. Record ready only when the candidate wins within the selected intervention level; otherwise record revise and keep the source as the acceptance baseline.` : "Fix the objective issue ledger with high-level Studio operations, rebuild the exact slide, and request critique again. Do not merely reduce all type sizes.",
        };
      }
      ...
      if (request.operation === "record_studio_visual_critique") {
        ...
        const rawSourceComparison = request.input.sourceComparison && typeof request.input.sourceComparison === "object" ? request.input.sourceComparison as Record<string, unknown> : {};
        const sourceComparison = {
          preferred: String(rawSourceComparison.preferred ?? "source") as "source" | "candidate" | "equivalent",
          sourceStrengths: Array.isArray(rawSourceComparison.sourceStrengths) ? rawSourceComparison.sourceStrengths.map(String).map((value) => value.trim().slice(0, 500)).filter(Boolean).slice(0, 12) : [],
          candidateImprovements: Array.isArray(rawSourceComparison.candidateImprovements) ? rawSourceComparison.candidateImprovements.map(String).map((value) => value.trim().slice(0, 500)).filter(Boolean).slice(0, 12) : [],
          candidateRegressions: Array.isArray(rawSourceComparison.candidateRegressions) ? rawSourceComparison.candidateRegressions.map(String).map((value) => value.trim().slice(0, 500)).filter(Boolean).slice(0, 12) : [],
        };
        const pass = critique.iteration.currentPass;
        const serious = critique.issues.some((issue) => issue.severity !== "minor") || visualIssues.some((issue) => issue.severity !== "minor") || sourceComparison.preferred === "source";
      ...
      if (request.operation === "get_slide_measurements") {
        ...
        const representation = request.input.representation === "proposal" ? "proposal" as const : "current" as const;
        if (!Number.isInteger(slideNumber) || slideNumber < 1 || slideNumber > deck.audit.slideCount) throw new Error(`Choose a slide from 1 to ${deck.audit.slideCount}.`);
        const measurement = await getOrBuildNativeSlideMeasurement(deck, slideNumber, representation, current);
        const baseline = representation === "proposal" ? await getOrBuildNativeSlideMeasurement(deck, slideNumber, "current", current) : undefined;
      ...
      if (request.operation === "record_proposal_visual_critique") {
        ...
        const [currentRender, proposalRender, currentMeasurement, proposalMeasurement] = await Promise.all([
          getOrBuildInspectionSlideRender(deck, slideNumber, "current", current),
          getOrBuildInspectionSlideRender(deck, slideNumber, "proposal", current),
          getOrBuildNativeSlideMeasurement(deck, slideNumber, "current", current),
          getOrBuildNativeSlideMeasurement(deck, slideNumber, "proposal", current),
        ]);
        const currentSlide = currentRender.slides.find((slide) => slide.number === slideNumber);
        const proposalSlide = proposalRender.slides.find((slide) => slide.number === slideNumber);
      ...
      if (request.operation === "stage_studio_web_design") {
        ...
        const rawArchetype = typeof request.input.archetype === "string" ? request.input.archetype : undefined;
        if (rawArchetype && !STUDIO_DESIGN_ARCHETYPES.includes(rawArchetype as StudioDesignArchetype)) throw new Error("Choose a supported Studio communication archetype.");
        if (!templateCatalog) throw new Error("Install an authorized PowerPoint Template Pack before staging an ORNL Studio composition.");
        const auditedSlide = deck.audit.slides.find((slide) => slide.number === slideNumber);
        const compositionPlan = planStudioComposition(contentProfileForSlide(deck, slideNumber), templateCatalog.layouts, {
          slideNumber,
          title: auditedSlide?.title,
          connectorCount: studioSourceSlide.nodes.filter((node) => node.kind === "connector").length,
          nativeObjectCount: studioSourceSlide.nodes.filter((node) => node.kind === "native-object").length,
          repeatedImageSeries: Boolean(inferRepeatedImageSeries(studioSourceSlide)),
          recommendedRecipe: recommendedStudioRecipe(studioSourceSlide),
          requestedArchetype: rawArchetype as StudioDesignArchetype | undefined,
        });
        const recipe = String(request.input.recipe ?? compositionPlan.recipe) as StudioLayoutRecipe;
        if (!allowedRecipes.includes(recipe)) throw new Error("Choose a supported Studio web recipe or omit it to use the communication-archetype plan.");
        const layoutId = typeof request.input.layoutId === "string" ? request.input.layoutId : recipe === "template-layout" ? compositionPlan.layoutId : undefined;
        const layout = layoutId ? templateCatalog.layouts.find((item) => item.id === layoutId) : undefined;
        if (recipe === "template-layout" && !layout) throw new Error("No exact compatible installed Template Pack layout was found. Use the recommended shared Studio archetype or choose a valid layout ID explicitly.");
        const rationale = String(request.input.rationale ?? compositionPlan.reasons.join(" "));
        const rawIntervention = typeof request.input.interventionLevel === "string" ? request.input.interventionLevel : undefined;
        if (rawIntervention && !STUDIO_INTERVENTION_LEVELS.includes(rawIntervention as StudioInterventionLevel)) throw new Error("Choose preserve, polish, recompose, or rebuild-figure as the slide intervention level.");
        const interventionLevel = (rawIntervention ?? resolveStudioIntervention(deck, slideNumber, studioSourceSlide, compositionPlan.archetype).level) as StudioInterventionLevel;
        if (interventionLevel === "preserve" && recipe !== "source") throw new Error("A preserve intervention must retain the source composition. Choose recipe source or use polish/recompose for a changed candidate.");
        let studioScene = recomposeStudioWebSlide(baseScene, slideNumber, recipe, layout, rationale);
        ...
        const interventionReason = String(request.input.interventionRationale ?? rationale).trim().slice(0, 1_000);
        studioScene = {
          ...studioScene,
          slides: studioScene.slides.map((slide) => slide.slideNumber !== slideNumber ? slide : withStudioIntervention(
            { ...slide, designArchetype: compositionPlan.archetype },
            interventionLevel,
            interventionReason || `Use the ${interventionLevel} intervention for this ${compositionPlan.archetype} slide.`,
            rawIntervention ? "ai" : "automatic",
          )),
        };
        ...
        const compilerMode = "fresh-composition" as const;
        {
          ...
          return {
            ...
            compilerMode,
            compositionPlan,
            slide: { number: slideNumber, archetype: compositionPlan.archetype, interventionLevel, recipe, layoutId: layout?.id, layoutName: layout?.name, nodeFrameOverrideCount: nodeFrames.length, nodeStyleOverrideCount: nodeStyles.length, figureTreatmentCount: figureTreatments.length, reconstructedVisualNeedCount: visualNeedIds.length },
            applied: false,
            saved: false,
      ...
      if (request.operation === "stage_designer_cleanup") {
        ...
        const proposal = createDesignerCleanupProposal(designerDeck, current.project.updatedAt);
        const next = touchProject({
          ...current,
          decks: current.decks.map((item) => item.id === deck.id ? { ...designerDeck, proposal, status: "proposal-ready" as const } : item),
      ...
      if (["solve_and_stage_alignment", "solve_and_stage_distribution", "solve_and_stage_safe_region", "solve_and_stage_group_layout", "fit_scene_to_layout"].includes(request.operation)) {
        ...
        if (deckTemplateWorkflow(deck) === "source-template-cleanup" && ["solve_and_stage_group_layout", "fit_scene_to_layout"].includes(request.operation)) throw new Error("This sponsor/custom deck must preserve its native layout. Use source-bound alignment, distribution, safe-region, measured text-fit, geometry, or table-layout operations unless the user explicitly chooses ORNL cross-template conversion.");
        const slideNumber = Number(request.input.slideNumber);
        ...
        const rationale = String(request.input.rationale ?? "").trim();
        if (!Number.isInteger(slideNumber) || slideNumber < 1 || slideNumber > deck.audit.slideCount) throw new Error(`Choose a slide from 1 to ${deck.audit.slideCount}.`);
        const measurement = await getOrBuildNativeSlideMeasurement(deck, slideNumber, "current", current);
        ...
        const proposalMeasurement = await getOrBuildNativeSlideMeasurement(proposedDeck, slideNumber, "proposal", current);
      ...
      if (request.operation === "solve_and_stage_text_fit") {
        ...
        const measurement = await getOrBuildNativeSlideMeasurement(deck, object.slideNumber, "current", current);
        ...
        const proposedDeck = { ...deck, proposal };
        const proposalMeasurement = await getOrBuildNativeSlideMeasurement(proposedDeck, object.slideNumber, "proposal", current);
        ...
        const nativeRender = await getOrBuildInspectionSlideRender(proposedDeck, object.slideNumber, "proposal", current);
      ...
      if (request.operation === "solve_and_stage_table_layout") {
        ...
        const tableInventory = deck.audit.tables.find((table) => table.id === tableId);
        if (!tableInventory) throw new Error("The requested table is stale. Read a fresh inspection packet and select its current stable table ID.");
        const tableSlideNumber = tableInventory.slideNumber;
        const measurement = await getOrBuildNativeSlideMeasurement(deck, tableSlideNumber, "current", current);
        ...
        for (let iteration = 1; iteration <= 3; iteration += 1) {
          ...
          proposalMeasurement = await getOrBuildNativeSlideMeasurement({ ...deck, proposal }, tableSlideNumber, "proposal", current);
        ...
        const nativeRender = await getOrBuildInspectionSlideRender({ ...deck, proposal }, tableSlideNumber, "proposal", current);
        const proposalSlideNumber = tableSlideNumber;
      ...
      if (request.operation === "stage_slide_visual_design") {
        ...
        if (deckTemplateWorkflow(deck) === "source-template-cleanup") throw new Error("ORNL visual styling and decoration are unavailable while this sponsor/custom source template is being preserved. Use source-bound geometry and fit operations instead.");
        const slideNumber = Number(request.input.slideNumber);
      ...
      if (request.operation === "stage_slide_native_layout") {
        ...
        if (deckTemplateWorkflow(deck) === "source-template-cleanup") throw new Error("This sponsor/custom deck must preserve its current native layout. ORNL Template Pack remapping requires an explicit cross-template conversion decision.");
        const source = sourceForDeck(current, deck);
      ...
      if (request.operation === "stage_slide_recomposition") {
        ...
        if (deckTemplateWorkflow(deck) === "source-template-cleanup") throw new Error("This sponsor/custom deck must preserve its current native layout. ORNL semantic recomposition requires an explicit cross-template conversion decision.");
        const slideNumber = Number(request.input.slideNumber);
        const layoutId = String(request.input.layoutId ?? "");
        const layout = templateCatalog.layouts.find((item) => item.id === layoutId);
      ...
      if ((changedProject !== commandStartProject || changedProject.project.updatedAt !== commandStartRevision) && changedProject.settings.autosave && projectHasRecoverableWork(changedProject)) {
        const snapshot = { project: changedProject, password: secureAutosavePassword };
        const resourceInventoryChanged = projectResourceInventoryKey(changedProject) !== projectResourceInventoryKey(commandStartProject);
        checkpointSaverRef.current?.request(snapshot);
        packageSaverRef.current?.request(snapshot);
        if (!checkpointSaverRef.current) throw new Error("The AI design changed the project, but the recovery checkpoint service is unavailable.");
        await checkpointSaverRef.current.flush();
        if (resourceInventoryChanged) {
          if (!packageSaverRef.current) throw new Error("The AI design added or changed embedded Resources, but the complete recovery package service is unavailable.");
          await packageSaverRef.current.flush();
        }
        if (typeof result === "object" && result) response = { ...result, recoverySaved: true, recoveryRevision: changedProject.project.updatedAt, recoveryLevel: resourceInventoryChanged ? "complete-self-contained-package" : "lightweight-project-checkpoint" };
      }
  ...
  }, [desktop, getOrBuildNativeRender, getOrBuildProposalCatalog, getOrBuildSlideCatalog, getOrBuildTemplateNativeRender, mcpEnabled, presentationFontCss, secureAutosavePassword, selectedDeckId, templateCatalog, templateNativeLoading, templateNativeRender]);
  ...
  function resetProjectRenderState() {
    ...
    automaticStudioBuildAttempted.current.clear();
  }

  function clearDeckRenderState(deckId: string) {
    for (const cache of [slideCatalogsRef.current, proposalCatalogsRef.current, nativeRenderCatalogsRef.current, nativeMeasurementsRef.current, inspectionRendersRef.current]) {
      for (const key of [...cache.keys()]) if (key === deckId || key.startsWith(`${deckId}:`)) cache.delete(key);
    }
    const withoutDeck = <T,>(catalog: Record<string, T>) => Object.fromEntries(Object.entries(catalog).filter(([key]) => key !== deckId && !key.startsWith(`${deckId}:`))) as Record<string, T>;
    setSlideCatalogs((catalog) => withoutDeck(catalog));
    setProposalCatalogs((catalog) => withoutDeck(catalog));
    ...
    setStudioDeckQualifications(studioDeckQualificationsRef.current);
    setSlideCatalogLoadingDeckId((id) => id === deckId ? undefined : id);
    setProposalCatalogLoadingDeckId((id) => id === deckId ? undefined : id);
    setNativeRenderLoadingKey((key) => key?.startsWith(`${deckId}:`) ? undefined : key);
    setSlideWorkspaceRequest((request) => request?.deckId === deckId ? undefined : request);
    for (const key of [...automaticStudioBuildAttempted.current]) if (key === deckId || key.startsWith(`${deckId}:`)) automaticStudioBuildAttempted.current.delete(key);
  }
  ...
  async function openProjectFile(file: PickedBinaryFile) {
    ...
    try {
      ...
      if (opened.decks.some(deckAuditNeedsUpgrade)) setBusy(`Updating saved PowerPoint audits in ${file.name}…`);
      adoptOpenedProject(await upgradeOpenedProjectAudits(opened), password);
    } catch (caught) {
  ...
  function confirmTemplate(templateId: string) {
    ...
    const now = new Date().toISOString();
    clearDeckRenderState(selectedDeck.id);
    setProject((current) => {
      const next = touchProject({ ...current, decks: current.decks.map((deck) => deck.id === selectedDeck.id ? withCompiledScene({ ...deck, operationScope: "reflow", targetTemplateId: templateId, targetTemplateConfirmedAt: now, targetTemplateDecisionSource: "user-selected", designProfile: templateId === PRESENTATION_DESIGN_STANDARD.defaults.template.id ? createOrnlDesignProfile("user-selected", now) : undefined, status: templateId === PRESENTATION_DESIGN_STANDARD.defaults.template.id ? "ready-for-cleanup" : "audited", studioScene: undefined, proposal: undefined }) : deck) }, "target-template-confirmed", `Confirmed ${templateId} for ${selectedDeck.name}; stale proposals and alternate Studio compositions were cleared.`);
      projectRef.current = next;
      return next;
    });
    setNotice(templateId === PRESENTATION_DESIGN_STANDARD.defaults.template.id ? "ORNL is now the design target. Create the central Studio scene and rebuild the deck with approved ORNL layouts; source bytes remain unchanged." : "Source-template preservation was explicitly selected. ORNL conversion is paused; source bytes remain unchanged.");
  }
  ...
  async function initializeStudioScene() {
    ...
    try {
      ...
      const adopted = {
        ...deck,
        operationScope: "reflow" as const,
        ...
        targetTemplateDecisionSource: sourceTemplateMode ? "user-selected" as const : deck.targetTemplateDecisionSource ?? "automatic-default" as const,
  ...
  function recomposeStudioSlide(slideNumber: number, recipe: StudioLayoutRecipe, layoutId?: string) {
    ...
    try {
      ...
      const layout = layoutId ? templateCatalog?.layouts.find((item) => item.id === layoutId) : undefined;
      if (isProtectedOrnlTemplateSlide(selectedDeck, slideNumber)) throw new Error(`The approved ORNL template composition on slide ${slideNumber} is sacred and locked. Choose another slide to redesign.`);
  ...
  function startCentralStudioDeckBuild(deck: DeckJob, studioScene: StudioWebScene): StudioDeckBuildJob {
    ...
    void getOrBuildCentralStudioDeck(
      ...
      setStudioDeckBuilds(studioDeckBuildsRef.current);
      updateStudioDeckBuildJob(deck.id, job.id, { phase: "ready", progressPercent: 100, message: `All ${build.outputSlides.length} candidate slides passed technical PowerPoint validation and are ready for visual inspection.`, completedAt: new Date().toISOString(), slideCount: build.slideCount });
      setNotice(`Built ${build.outputSlides.length} exact editable PowerPoint candidates. The faithful source remains the public result until the revision-bound visual review accepts these pixels.`);
    }).catch((caught) => {
      const activeJob = studioDeckBuildJobsRef.current[deck.id];
      if (!activeJob || activeJob.id !== job.id) return;
  ...
  useEffect(() => {
    if (activeView !== "slides" || !desktop || !selectedDeck?.audit || !selectedDeck.studioScene) return;
    if (selectedDeck.proposal?.status === "applied" || isProposalSlideWorkspaceRequest(slideWorkspaceRequest, selectedDeck.id)) return;
  ...
  async function qualifyStudioDeck(deck: DeckJob, build: StudioDeckBuild): Promise<StudioDeckQualification> {
    ...
    const interventionBySlide = Object.fromEntries(build.outputSlides.map((output) => {
      const sourceSlide = deck.studioScene!.slides.find((slide) => slide.slideNumber === output.sourceSlideNumber);
    ...
    const report = buildDeckQualificationReport({
      ...
      designImpactBySlide,
      interventionBySlide,
      deckConsistencyIssues: deckConsistency.issues,
      visualNeedBySlide,
  ...
  async function saveFreshStudioSlide(slideNumber: number) {
    ...
    const rendered = preview.nativeRender.slides[0];
    const review = studioSlide.qualityReview;
    if (!rendered || review?.recordedVerdict !== "ready" || review.slideUpdatedAt !== studioSlide.updatedAt || review.rasterSha256 !== rendered.sha256) { setError("This exact PowerPoint candidate has not passed revision-bound visual review. Inspect it, record the ready verdict, and then save it."); return; }
    clearMessages();
    try {
  ...
  async function saveCentralStudioDeck() {
    ...
    const qualification = studioDeckQualificationsRef.current[selectedDeck.id];
    if (!qualification || qualification.sceneRevision !== selectedDeck.studioScene.revision || qualification.report.status !== "review-complete") { setError("Export is held until every exact candidate slide passes the PowerPoint-native visual review. Run Inspect all and complete the Found issues → Fixing → Rechecking loop first."); return; }
    clearMessages();
    setBusy("Rerendering the exact central presentation export candidate in Microsoft PowerPoint…");
    try {
      if (qualification.candidateSha256 !== await sha256(build.bytes)) throw new Error("The central PowerPoint candidate changed after visual review. Rebuild and reinspect the exact current artifact before exporting.");
  ...
  function stageCleanup() {
    ...
    clearMessages();
    try {
      const designerDeck = withDesignerCleanupScope(selectedDeck);
      const proposal = createDesignerCleanupProposal(designerDeck, project.project.updatedAt);
      setProject((current) => ({
        ...current,
        decks: current.decks.map((deck) => deck.id === selectedDeck.id ? { ...designerDeck, proposal, status: "proposal-ready" } : deck),
  ...
  const currentStudioQualification = selectedDeck?.studioScene && studioDeckQualifications[selectedDeck.id]?.sceneRevision === selectedDeck.studioScene.revision
    ...
    : undefined;
  const acceptedCentralStudioBuild = currentStudioDeckBuild && currentStudioQualification?.report.status === "review-complete"
    ? currentStudioDeckBuild
    : undefined;
  const latestStudioNativeRender = acceptedCentralStudioBuild?.nativeRender ?? (selectedDeck?.studioScene
  ...
  }, [acceptedCentralStudioBuild, activeView, currentStudioDeckBuild, currentStudioQualification, latestStudioNativeRender, mcpEnabled, nativeRenderCatalogs, nativeRenderLoadingKey, project, proposalCatalogLoadingDeckId, proposalCatalogs, selectedDeck, slideCatalogLoadingDeckId, slideCatalogs, slideWorkspaceRequest, studioDeckBuildLoadingId, studioFreshPreviews, studioOpenSlideNumber, templateCatalog, templateInstalledAt, templateLoading, templateNativeLoading, templateNativeRender]);
  const mcpActivityMessage = mcpActivity ? mcpActivityCopy(mcpActivity) : undefined;
  const autosaveError = autosaveUi.checkpoint?.phase === "error" ? autosaveUi.checkpoint.error : autosaveUi.package?.phase === "error" ? autosaveUi.package.error : undefined;
  const checkpointCurrent = autosaveUi.checkpoint?.phase === "saved" && autosaveUi.checkpoint.revision === project.project.updatedAt;
  const packageCurrent = autosaveUi.package?.phase === "saved" && autosaveUi.package.revision === project.project.updatedAt;
  const autosaveWorking = [autosaveUi.checkpoint?.phase, autosaveUi.package?.phase].some((phase) => phase === "pending" || phase === "saving");
  const autosaveLabel = autosaveError ? "Autosave needs attention" : packageCurrent ? "All changes saved" : checkpointCurrent ? "Recovery saved · packaging" : autosaveWorking ? "Saving changes" : projectHasRecoverableWork(project) ? "Autosave ready" : "Not saved yet";
  const autosavePhase = autosaveError ? "error" : packageCurrent || checkpointCurrent ? "saved" : autosaveWorking ? "saving" : "idle";
