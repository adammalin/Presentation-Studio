  ...
  useEffect(() => {
    ...
    return desktop.onMcpCommand(async (request) => {
      ...
      if (request.operation === "get_studio_web_scene") {
        ...
        return {
          ...
          instruction: slide.contentCoverage.exactTextMapped
            ? "Design in this semantic HTML/CSS scene, not by preserving weak source coordinates. Source paragraphs are exposed as exact semantic atom candidates, but approved wording remains locked. Choose one shared ORNL recipe or installed template layout, then use stage_studio_web_design with compilerMode fresh-composition for a new composed slide or source-bound-overlay when native source internals must survive. For fresh composition, call preview_studio_fresh_composition with the returned projectUpdatedAt and judge the Microsoft PowerPoint render before any save."
      ...
      if (request.operation === "stage_studio_web_design") {
        ...
        const slideNumber = Number(request.input.slideNumber);
        if (!Number.isInteger(slideNumber) || slideNumber < 1 || slideNumber > deck.audit.slideCount) throw new Error(`Choose a slide from 1 to ${deck.audit.slideCount}.`);
        const allowedRecipes: StudioLayoutRecipe[] = ["source", "ornl-title-content", "ornl-title-two-column", "ornl-title-card-grid", "ornl-title-table", "ornl-title-figure-grid", "template-layout"];
