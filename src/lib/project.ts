import { z } from "zod";
import {
  PRESENTATION_SCENE_SCHEMA,
  PRESENTATION_SCENE_VERSION,
  PRESERVATION_ENVELOPE_SCHEMA,
  PRESERVATION_ENVELOPE_VERSION,
  STUDIO_WEB_SCENE_SCHEMA,
  STUDIO_WEB_SCENE_VERSION,
  PROJECT_SCHEMA,
  PROJECT_SCHEMA_VERSION,
  type PresentationStudioProject,
} from "../types";
import { defaultProjectDesignSettings } from "./design-standard";

const isoTimestamp = z.string().datetime({ offset: true });
const fontSchema = z.object({
  family: z.string(),
  normalizedFamily: z.string(),
  count: z.number().int().nonnegative(),
  directSlideCount: z.number().int().nonnegative(),
  slideNumbers: z.array(z.number().int().positive()),
  partKinds: z.array(z.string()),
  isThemeFont: z.boolean(),
  isLikelySymbolFont: z.boolean(),
});
const slideSchema = z.object({
  id: z.string(),
  number: z.number().int().positive(),
  sourcePart: z.string().optional(),
  sourcePartSha256: z.string().regex(/^[0-9a-f]{64}$/).optional(),
  relationshipPart: z.string().optional(),
  relationshipPartSha256: z.string().regex(/^[0-9a-f]{64}$/).optional(),
  title: z.string(),
  text: z.string(),
  textHash: z.string().regex(/^[0-9a-f]{64}$/),
  textRunCount: z.number().int().nonnegative(),
  tableCount: z.number().int().nonnegative(),
  pictureCount: z.number().int().nonnegative(),
  chartCount: z.number().int().nonnegative(),
  connectorCount: z.number().int().nonnegative(),
  commentCount: z.number().int().nonnegative(),
  fonts: z.array(z.string()),
  fontSizes: z.array(z.number().nonnegative()),
  warnings: z.array(z.string()),
});
const tableInventorySchema = z.object({
  id: z.string(),
  slideNumber: z.number().int().positive(),
  ordinal: z.number().int().positive(),
  rowCount: z.number().int().nonnegative(),
  columnCount: z.number().int().nonnegative(),
  mergedCellCount: z.number().int().nonnegative(),
  totalCellCharacterCount: z.number().int().nonnegative().default(0),
  maximumCellCharacterCount: z.number().int().nonnegative().default(0),
  styleId: z.string().optional(),
  styleFlags: z.array(z.string()),
  cellFonts: z.array(z.string()),
  colorTokens: z.array(z.string()),
  semanticColorTokens: z.array(z.string()).default([]),
  marginSignatures: z.array(z.string()),
  styleFingerprint: z.string().regex(/^[0-9a-f]{64}$/),
  contentHash: z.string().regex(/^[0-9a-f]{64}$/).default("0".repeat(64)),
  structureHash: z.string().regex(/^[0-9a-f]{64}$/).default("0".repeat(64)),
  columns: z.array(z.object({ id: z.string(), index: z.number().int().positive(), widthEmu: z.number().int().nonnegative() })).default([]),
  rows: z.array(z.object({ id: z.string(), index: z.number().int().positive(), heightEmu: z.number().int().nonnegative() })).default([]),
  cells: z.array(z.object({
    id: z.string(),
    row: z.number().int().positive(),
    column: z.number().int().positive(),
    rowSpan: z.number().int().positive(),
    columnSpan: z.number().int().positive(),
    horizontalMergeContinuation: z.boolean(),
    verticalMergeContinuation: z.boolean(),
    text: z.string().default(""),
    textRuns: z.array(z.string()).optional(),
    paragraphRunCounts: z.array(z.number().int().nonnegative()).optional(),
    runBreaksBefore: z.array(z.enum(["none", "line", "paragraph"])).optional(),
    textHash: z.string().regex(/^[0-9a-f]{64}$/),
    characterCount: z.number().int().nonnegative(),
    paragraphCount: z.number().int().positive(),
    fontFamilies: z.array(z.string()),
    fontSizes: z.array(z.number().nonnegative()),
    fillToken: z.string().optional(),
    semanticColorRole: z.string().optional(),
    marginsEmu: z.object({ left: z.number().int().nonnegative(), right: z.number().int().nonnegative(), top: z.number().int().nonnegative(), bottom: z.number().int().nonnegative() }),
    horizontalAlignment: z.enum(["left", "center", "right", "justified", "mixed"]),
    verticalAlignment: z.enum(["top", "middle", "bottom"]),
  })).default([]),
});
const pictureInventorySchema = z.object({
  id: z.string(),
  slideNumber: z.number().int().positive(),
  ordinal: z.number().int().positive(),
  name: z.string(),
  description: z.string().optional(),
  relationshipId: z.string().optional(),
  widthEmu: z.number().int().nonnegative().optional(),
  heightEmu: z.number().int().nonnegative().optional(),
  cropped: z.boolean(),
  hasOutline: z.boolean(),
  hasEffect: z.boolean(),
});
const geometrySchema = z.object({ x: z.number().int(), y: z.number().int(), width: z.number().int().nonnegative(), height: z.number().int().nonnegative() });
const editableObjectSchema = z.object({
  id: z.string(),
  slideNumber: z.number().int().positive(),
  shapeId: z.string(),
  name: z.string(),
  kind: z.enum(["text", "shape", "picture", "table", "chart", "connector", "group", "graphic-frame"]),
  sourceElement: z.enum(["p:sp", "p:pic", "p:graphicFrame", "p:cxnSp", "p:grpSp"]),
  geometry: geometrySchema.extend({ rotation: z.number() }),
  canMove: z.boolean(),
  canResize: z.boolean(),
  textHash: z.string().regex(/^[0-9a-f]{64}$/).optional(),
  tableId: z.string().optional(),
  pictureId: z.string().optional(),
});
const sceneFidelityStateSchema = z.enum(["editable-native", "preserved-native", "conversion-required", "unsupported-blocking"]);
const sceneFidelityCountsSchema = z.object({
  "editable-native": z.number().int().nonnegative(),
  "preserved-native": z.number().int().nonnegative(),
  "conversion-required": z.number().int().nonnegative(),
  "unsupported-blocking": z.number().int().nonnegative(),
});
const sceneObjectSchema = z.object({
  id: z.string().min(1),
  slideId: z.string().min(1),
  slideNumber: z.number().int().positive(),
  shapeId: z.string().min(1),
  name: z.string(),
  kind: z.enum(["text", "shape", "picture", "table", "chart", "connector", "group", "graphic-frame"]),
  sourceElement: z.enum(["p:sp", "p:pic", "p:graphicFrame", "p:cxnSp", "p:grpSp"]),
  semanticRole: z.enum(["title", "body", "caption", "label", "image", "table", "chart", "connector", "group", "decoration", "other"]),
  fidelityState: sceneFidelityStateSchema,
  fidelityReason: z.string().min(1),
  geometry: geometrySchema.extend({ rotation: z.number() }),
  zIndex: z.number().int().nonnegative(),
  sourceLocator: z.object({
    slidePart: z.string().min(1),
    shapeId: z.string().min(1),
    tableId: z.string().optional(),
    pictureId: z.string().optional(),
  }),
  representation: z.object({
    geometry: z.enum(["native", "partial", "preserved", "none"]),
    text: z.enum(["native", "partial", "preserved", "none"]),
    style: z.enum(["native", "partial", "preserved", "none"]),
    internalStructure: z.enum(["native", "partial", "preserved", "none"]),
  }),
  operations: z.object({
    move: z.boolean(),
    resize: z.boolean(),
    restyle: z.boolean(),
    editText: z.boolean(),
    editTableStyle: z.boolean(),
    replaceMedia: z.boolean(),
    editChartData: z.boolean(),
    editInternalStructure: z.boolean(),
  }),
  contentHash: z.string().regex(/^[0-9a-f]{64}$/).optional(),
  protected: z.boolean(),
});
const sceneSlideSchema = z.object({
  id: z.string().min(1),
  number: z.number().int().positive(),
  sourcePart: z.string().min(1),
  sourcePartSha256: z.string().regex(/^[0-9a-f]{64}$/).optional(),
  relationshipPart: z.string().optional(),
  relationshipPartSha256: z.string().regex(/^[0-9a-f]{64}$/).optional(),
  sourceTextHash: z.string().regex(/^[0-9a-f]{64}$/),
  targetLayoutId: z.string().optional(),
  objectIds: z.array(z.string().min(1)),
  fidelityCounts: sceneFidelityCountsSchema,
  preservationRequired: z.boolean(),
  protected: z.boolean(),
});
const sceneTableSchema = z.object({
  id: z.string().min(1),
  objectId: z.string().min(1),
  slideNumber: z.number().int().positive(),
  rowCount: z.number().int().nonnegative(),
  columnCount: z.number().int().nonnegative(),
  geometry: geometrySchema,
  rows: z.array(z.object({ id: z.string().min(1), index: z.number().int().positive(), height: z.number().int().nonnegative(), y: z.number().int().nonnegative() })),
  columns: z.array(z.object({ id: z.string().min(1), index: z.number().int().positive(), width: z.number().int().nonnegative(), x: z.number().int().nonnegative() })),
  cells: z.array(z.object({
    id: z.string().min(1),
    row: z.number().int().positive(),
    column: z.number().int().positive(),
    rowSpan: z.number().int().positive(),
    columnSpan: z.number().int().positive(),
    geometry: geometrySchema,
    margins: z.object({ left: z.number().int().nonnegative(), right: z.number().int().nonnegative(), top: z.number().int().nonnegative(), bottom: z.number().int().nonnegative() }),
    contentHash: z.string().regex(/^[0-9a-f]{64}$/),
    characterCount: z.number().int().nonnegative(),
    horizontalAlignment: z.enum(["left", "center", "right", "justified", "mixed"]),
    verticalAlignment: z.enum(["top", "middle", "bottom"]),
    mergeContinuation: z.boolean(),
  })),
  contentHash: z.string().regex(/^[0-9a-f]{64}$/),
  structureHash: z.string().regex(/^[0-9a-f]{64}$/),
});
const preservationSlideSchema = z.object({
  slideId: z.string().min(1),
  slideNumber: z.number().int().positive(),
  sourcePart: z.string().min(1),
  sourcePartSha256: z.string().regex(/^[0-9a-f]{64}$/).optional(),
  relationshipPart: z.string().optional(),
  relationshipPartSha256: z.string().regex(/^[0-9a-f]{64}$/).optional(),
  sourceTextHash: z.string().regex(/^[0-9a-f]{64}$/),
  objectIds: z.array(z.string().min(1)),
});
const presentationSceneSchema = z.object({
  schema: z.literal(PRESENTATION_SCENE_SCHEMA),
  version: z.literal(PRESENTATION_SCENE_VERSION),
  revision: z.string().min(1),
  sourceSha256: z.string().regex(/^[0-9a-f]{64}$/),
  slideSize: z.object({ width: z.number().int().positive(), height: z.number().int().positive() }),
  templateBinding: z.object({
    sourceClassification: z.enum(["current-ornl", "older-or-modified-ornl", "sponsor", "custom", "mixed", "unknown"]),
    targetTemplateId: z.string().optional(),
    targetDecisionSource: z.enum(["automatic-default", "automatic-source-preservation", "user-selected"]).optional(),
  }),
  slides: z.array(sceneSlideSchema),
  objects: z.array(sceneObjectSchema),
  tables: z.array(sceneTableSchema).default([]),
  fidelityCounts: sceneFidelityCountsSchema,
  preservationEnvelope: z.object({
    schema: z.literal(PRESERVATION_ENVELOPE_SCHEMA),
    version: z.literal(PRESERVATION_ENVELOPE_VERSION),
    sourceResourceId: z.string().min(1),
    sourceSha256: z.string().regex(/^[0-9a-f]{64}$/),
    sourceBytesAuthoritative: z.literal(true),
    nativeRenderAuthoritativeForAppearance: z.literal(true),
    exportStrategy: z.literal("surgical-ooxml-overlay"),
    packageFileCount: z.number().int().positive(),
    expandedByteLength: z.number().int().nonnegative(),
    protectedFeatures: z.object({ macros: z.boolean(), oleObjects: z.boolean(), externalRelationships: z.boolean() }),
    blockingFeatures: z.array(z.enum(["macros", "ole-objects", "external-relationships"])),
    slides: z.array(preservationSlideSchema),
  }),
});
const studioFrameSchema = z.object({ x: z.number().int(), y: z.number().int(), width: z.number().int().positive(), height: z.number().int().positive(), rotation: z.number() });
const studioResourceBindingSchema = z.object({
  resourceId: z.string().min(1),
  resourceSha256: z.string().regex(/^[0-9a-f]{64}$/),
  derivativeSha256: z.string().regex(/^[0-9a-f]{64}$/).optional(),
  kind: z.enum(["text", "image", "table"]),
  relationship: z.enum(["grounds", "supplies-media", "supplies-data"]),
  exactExcerpt: z.string().min(1).max(20_000).optional(),
  exactExcerptHash: z.string().regex(/^[0-9a-f]{64}$/).optional(),
});
const studioNodeSchema = z.object({
  id: z.string().min(1),
  sourceObjectId: z.string().min(1),
  sourceShapeId: z.string().min(1),
  sourceBinding: z.enum(["editable-object", "catalog-derived", "semantic-atom"]),
  name: z.string(),
  kind: z.enum(["text", "image", "table", "shape", "connector", "native-object"]),
  role: z.enum(["title", "body", "caption", "label", "image", "table", "chart", "connector", "group", "decoration", "other"]),
  sourceFrame: studioFrameSchema,
  frame: studioFrameSchema,
  zIndex: z.number().int().nonnegative(),
  sourceTextOrder: z.number().int().nonnegative(),
  visible: z.boolean().default(true),
  locked: z.boolean(),
  exactContent: z.boolean(),
  text: z.string().optional(),
  textHash: z.string().regex(/^[0-9a-f]{64}$/).optional(),
  sourceParagraphs: z.array(z.object({
    index: z.number().int().positive(),
    text: z.string(),
    textHash: z.string().regex(/^[0-9a-f]{64}$/),
    characterCount: z.number().int().nonnegative(),
    bullet: z.boolean(),
    bulletConfidence: z.enum(["direct", "inherited-possible"]),
    level: z.number().int().nonnegative(),
    fontFamilies: z.array(z.string()),
    fontSizes: z.array(z.number().nonnegative()),
  })).optional(),
  sourceAtom: z.object({
    sourceNodeId: z.string().min(1),
    sourceObjectId: z.string().min(1),
    paragraphStart: z.number().int().positive(),
    paragraphEnd: z.number().int().positive(),
    ordinal: z.number().int().nonnegative(),
    count: z.number().int().positive(),
    aggregateSourceTextHash: z.string().regex(/^[0-9a-f]{64}$/).optional(),
  }).optional(),
  resourceBindings: z.array(studioResourceBindingSchema).max(20).optional(),
  tableId: z.string().optional(),
  table: z.object({
    rows: z.number().int().nonnegative(),
    columns: z.number().int().nonnegative(),
    cells: z.array(z.object({ id: z.string().min(1), row: z.number().int().positive(), column: z.number().int().positive(), rowSpan: z.number().int().positive(), columnSpan: z.number().int().positive(), text: z.string(), textRuns: z.array(z.string()).optional(), paragraphRunCounts: z.array(z.number().int().nonnegative()).optional(), runBreaksBefore: z.array(z.enum(["none", "line", "paragraph"])).optional(), fill: z.string().optional(), semanticColorRole: z.string().optional() })),
    design: z.object({
      headerRows: z.number().int().nonnegative(),
      columnWidths: z.array(z.number().positive()),
      rowHeights: z.array(z.number().positive()),
      borderMode: z.enum(["none", "subtle", "full"]),
      borderColor: z.string(),
      borderWidthPt: z.number().nonnegative(),
      defaultPaddingPt: z.object({ top: z.number().nonnegative(), right: z.number().nonnegative(), bottom: z.number().nonnegative(), left: z.number().nonnegative() }),
      cellStyles: z.array(z.object({
        cellId: z.string().min(1),
        fill: z.string().optional(),
        color: z.string().optional(),
        fontSizePt: z.number().positive().optional(),
        fontWeight: z.union([z.literal(400), z.literal(600), z.literal(700)]).optional(),
        textAlign: z.enum(["left", "center", "right"]).optional(),
        verticalAlign: z.enum(["top", "middle", "bottom"]).optional(),
        paddingPt: z.object({ top: z.number().nonnegative(), right: z.number().nonnegative(), bottom: z.number().nonnegative(), left: z.number().nonnegative() }).optional(),
        borders: z.object({
          top: z.object({ type: z.enum(["none", "solid", "dash"]), color: z.string(), widthPt: z.number().min(0).max(6) }).optional(),
          right: z.object({ type: z.enum(["none", "solid", "dash"]), color: z.string(), widthPt: z.number().min(0).max(6) }).optional(),
          bottom: z.object({ type: z.enum(["none", "solid", "dash"]), color: z.string(), widthPt: z.number().min(0).max(6) }).optional(),
          left: z.object({ type: z.enum(["none", "solid", "dash"]), color: z.string(), widthPt: z.number().min(0).max(6) }).optional(),
        }).optional(),
      })),
    }).optional(),
  }).optional(),
  connector: z.object({
    fromNodeId: z.string().min(1),
    toNodeId: z.string().min(1),
    fromSide: z.enum(["top", "right", "bottom", "left", "center"]),
    toSide: z.enum(["top", "right", "bottom", "left", "center"]),
    stroke: z.string(),
    widthPt: z.number().positive(),
    dash: z.enum(["solid", "dash", "dashDot"]),
    beginArrow: z.enum(["none", "arrow", "diamond", "oval", "stealth", "triangle"]),
    endArrow: z.enum(["none", "arrow", "diamond", "oval", "stealth", "triangle"]),
    verificationStatus: z.literal("verified"),
  }).optional(),
  mediaPart: z.string().optional(),
  component: z.object({ groupId: z.string().min(1), role: z.enum(["eyebrow", "card-kicker", "card-heading", "card-body", "objective-body", "step-heading", "step-body", "figure-media", "figure-label", "figure-caption", "technical-annotation", "question-intro", "question-item", "challenge-assertion", "challenge-intro", "challenge-body", "process-icon", "process-input", "process-stage", "process-output", "supporting-copy", "footer-logo", "footer-meta"]), ordinal: z.number().int().nonnegative().optional(), frame: studioFrameSchema.optional(), definitionId: z.string().min(1).optional() }).optional(),
  opticalInsets: z.object({
    left: z.number().nonnegative(), top: z.number().nonnegative(), right: z.number().nonnegative(), bottom: z.number().nonnegative(),
    authority: z.enum(["scene-frame", "source-estimate", "powerpoint-native"]),
    basis: z.enum(["shape", "rendered-text", "active-image-content"]),
  }).optional(),
  style: z.object({
    fontFamily: z.literal("Aptos"),
    fontSizePt: z.number().positive(),
    fontWeight: z.union([z.literal(400), z.literal(600), z.literal(700)]),
    lineHeight: z.number().positive(),
    color: z.string(),
    background: z.string().optional(),
    borderColor: z.string().optional(),
    borderWidthPt: z.number().nonnegative(),
    textAlign: z.enum(["left", "center", "right"]),
    verticalAlign: z.enum(["top", "middle", "bottom"]),
    paddingPt: z.object({ top: z.number().nonnegative(), right: z.number().nonnegative(), bottom: z.number().nonnegative(), left: z.number().nonnegative() }),
    objectFit: z.enum(["contain", "cover"]).optional(),
  }),
});
const studioWebSceneSchema = z.object({
  schema: z.literal(STUDIO_WEB_SCENE_SCHEMA),
  version: z.literal(STUDIO_WEB_SCENE_VERSION),
  revision: z.string().min(1),
  deckId: z.string().min(1),
  sourceSha256: z.string().regex(/^[0-9a-f]{64}$/),
  slideSize: z.object({ width: z.number().int().positive(), height: z.number().int().positive() }),
  sourceSlideSize: z.object({ width: z.number().int().positive(), height: z.number().int().positive() }),
  rhythm: z.object({ safeMarginPt: z.number().positive(), gridPt: z.number().positive(), compactGapPt: z.number().nonnegative(), normalGapPt: z.number().nonnegative(), primaryGapPt: z.number().nonnegative(), captionGapPt: z.number().nonnegative(), titleContentGapPt: z.number().nonnegative() }).optional(),
  designMemory: z.array(z.object({
    contentSignature: z.string().min(1).max(300), recipe: z.enum(["source", "ornl-title-content", "ornl-title-two-column", "ornl-title-card-grid", "ornl-title-metric-grid", "ornl-title-table", "ornl-title-figure-grid", "ornl-title-objective-columns", "ornl-title-steps-evidence", "ornl-title-labeled-figure-grid", "ornl-title-question-diagram", "ornl-title-challenges-evidence", "ornl-title-process-flow", "template-layout"]), targetLayoutId: z.string().optional(), targetLayoutName: z.string().optional(),
    rhythm: z.object({ safeMarginPt: z.number().positive(), gridPt: z.number().positive(), compactGapPt: z.number().nonnegative(), normalGapPt: z.number().nonnegative(), primaryGapPt: z.number().nonnegative(), captionGapPt: z.number().nonnegative(), titleContentGapPt: z.number().nonnegative() }),
    adoptedFromSlideNumber: z.number().int().positive(), qualityRasterSha256: z.string().length(64), recordedAt: isoTimestamp,
  })).max(80).default([]),
  componentLibrary: z.array(z.object({
    id: z.string().min(1),
    name: z.string().min(1),
    role: z.enum(["eyebrow", "card-kicker", "card-heading", "card-body", "metric-card", "objective-body", "step-heading", "step-body", "figure-media", "figure-label", "figure-caption", "technical-annotation", "question-intro", "question-item", "challenge-assertion", "challenge-intro", "challenge-body", "process-icon", "process-input", "process-stage", "process-output", "supporting-copy", "footer-logo", "footer-meta"]),
    surface: z.enum(["light", "dark"]),
    sourceNodeId: z.string().min(1),
    adoptedFromSlideNumber: z.number().int().positive(),
    style: studioNodeSchema.shape.style,
    updatedAt: isoTimestamp,
  })).max(80).default([]),
  tableLibrary: z.array(z.object({
    id: z.string().min(1),
    name: z.string().min(1).max(120),
    sourceNodeId: z.string().min(1),
    adoptedFromSlideNumber: z.number().int().positive(),
    compatibility: z.object({ columns: z.number().int().positive(), headerRows: z.number().int().nonnegative(), headerStructure: z.string(), bodyStructure: z.string() }),
    tableStyle: z.object({
      columnWidths: z.array(z.number().positive()).min(1),
      borderMode: z.enum(["none", "subtle", "full"]),
      borderColor: z.string(),
      borderWidthPt: z.number().min(0).max(6),
      defaultPaddingPt: z.object({ top: z.number().min(0).max(36), right: z.number().min(0).max(36), bottom: z.number().min(0).max(36), left: z.number().min(0).max(36) }),
    }),
    roleStyles: z.object({
      header: studioNodeSchema.shape.table.unwrap().shape.design.unwrap().shape.cellStyles.element.omit({ cellId: true }),
      bodyOdd: studioNodeSchema.shape.table.unwrap().shape.design.unwrap().shape.cellStyles.element.omit({ cellId: true }),
      bodyEven: studioNodeSchema.shape.table.unwrap().shape.design.unwrap().shape.cellStyles.element.omit({ cellId: true }),
    }),
    updatedAt: isoTimestamp,
  })).max(80).default([]),
  tableContinuationPlans: z.array(z.object({
    id: z.string().min(1),
    sourceSlideNumber: z.number().int().positive(),
    tableNodeId: z.string().min(1),
    headerRows: z.number().int().nonnegative(),
    maximumBodyRowsPerSlide: z.number().int().min(1).max(40),
    policy: z.literal("repeat-header-rows"),
    status: z.enum(["ready", "blocked"]),
    segments: z.array(z.object({ ordinal: z.number().int().positive(), bodyRowStart: z.number().int().positive(), bodyRowEnd: z.number().int().positive(), repeatedHeaderRows: z.number().int().nonnegative(), sourceCellIds: z.array(z.string().min(1)) })).max(100),
    blockers: z.array(z.string().min(1).max(1_000)).max(50),
    rationale: z.string().min(1).max(1_000),
    createdAt: isoTimestamp,
  })).max(100).default([]),
  designSystem: z.object({ id: z.enum(["ornl-presentation-web-v1", "source-template-preservation-web-v1"]), standardVersion: z.string().min(1), unit: z.literal("emu"), renderer: z.literal("html-css"), exportTarget: z.literal("editable-powerpoint"), compilerModes: z.tuple([z.literal("source-bound-overlay"), z.literal("fresh-composition")]).default(["source-bound-overlay", "fresh-composition"]) }),
  slides: z.array(z.object({
    id: z.string().min(1), slideNumber: z.number().int().positive(), sourceSlideId: z.string().min(1), sourceTextHash: z.string().regex(/^[0-9a-f]{64}$/), sourceRevision: z.string().min(1),
    contentCoverage: z.object({ exactTextMapped: z.boolean(), sourceContentSignature: z.string().optional(), sourceCharacterCount: z.number().int().nonnegative(), mappedCharacterCount: z.number().int().nonnegative(), sourceTextBoxCount: z.number().int().nonnegative(), mappedTextNodeCount: z.number().int().nonnegative(), groupedOrUnsupportedTextPresent: z.boolean() }),
    recipe: z.enum(["source", "ornl-title-content", "ornl-title-two-column", "ornl-title-card-grid", "ornl-title-metric-grid", "ornl-title-table", "ornl-title-figure-grid", "ornl-title-objective-columns", "ornl-title-steps-evidence", "ornl-title-labeled-figure-grid", "ornl-title-question-diagram", "ornl-title-challenges-evidence", "ornl-title-process-flow", "template-layout"]),
    targetLayoutId: z.string().optional(), targetLayoutName: z.string().optional(), background: z.string(), status: z.enum(["imported", "designed"]), designRationale: z.string().max(1_000), resourceBindings: z.array(studioResourceBindingSchema).max(40).optional(),
    figureTreatments: z.array(z.object({
      id: z.string().min(1).max(180),
      nodeIds: z.array(z.string().min(1).max(180)).min(1).max(30),
      mode: z.enum(["preserve-as-unit", "preserve-and-frame", "hybrid-rebuild", "redraw-candidate"]),
      verificationStatus: z.enum(["source-locked", "needs-content-review", "verified"]),
      intentSummary: z.string().min(1).max(1_000),
      informationInventory: z.array(z.string().min(1).max(500)).min(1).max(40),
      invariants: z.array(z.string().min(1).max(500)).min(1).max(40),
      rationale: z.string().min(1).max(1_000),
      replacementResourceId: z.string().min(1).max(180).optional(),
      relationships: z.array(z.object({ fromNodeId: z.string().min(1).max(180), toNodeId: z.string().min(1).max(180), kind: z.enum(["caption-for", "label-for", "callout-for", "connects-from", "connects-to", "contained-by"]) })).max(120).optional(),
      groupFrame: z.object({ x: z.number(), y: z.number(), width: z.number().positive(), height: z.number().positive(), rotation: z.number() }).optional(),
      focalPoint: z.object({ x: z.number().min(0).max(1), y: z.number().min(0).max(1) }).optional(),
      crop: z.object({ left: z.number().min(0).max(1), top: z.number().min(0).max(1), right: z.number().min(0).max(1), bottom: z.number().min(0).max(1) }).optional(),
      relationshipPolicy: z.enum(["preserve-internal", "reflow-annotations", "editable-diagram"]).optional(),
      lockAspectRatio: z.boolean().optional(),
    })).default([]),
    conceptReferences: z.array(z.object({
      id: z.string().min(1).max(180),
      resourceId: z.string().min(1).max(180),
      resourceSha256: z.string().regex(/^[0-9a-f]{64}$/),
      sourceTextHash: z.string().regex(/^[0-9a-f]{64}$/),
      status: z.literal("concept-only"),
      origin: z.enum(["imagegen", "human-reference", "other"]),
      approvedInfluences: z.array(z.enum(["composition", "visual-hierarchy", "negative-space", "color-balance", "figure-concept", "image-treatment", "visual-rhythm"])).min(1).max(7),
      untrustedElements: z.array(z.enum(["generated-text", "generated-logos", "generated-data", "generated-technical-details", "generated-claims"])).min(5).max(5),
      blueprint: z.object({
        summary: z.string().min(1).max(1_000),
        zones: z.array(z.object({
          id: z.string().min(1).max(120), role: z.enum(["title", "primary-visual", "supporting-evidence", "caption", "footer-safe", "other"]),
          x: z.number().min(0).max(1), y: z.number().min(0).max(1), width: z.number().positive().max(1), height: z.number().positive().max(1),
        })).max(20),
        styleNotes: z.array(z.string().min(1).max(500)).max(20),
        reconstructionNotes: z.array(z.string().min(1).max(500)).max(20),
      }),
      provenance: z.object({ model: z.string().min(1).max(180).optional(), promptSummary: z.string().min(1).max(1_000).optional(), generatedAt: isoTimestamp.optional() }).optional(),
      visualNeedId: z.string().min(1).max(180).optional(),
      attachedAt: isoTimestamp,
    })).max(8).default([]),
    visualNeeds: z.array(z.object({
      id: z.string().min(1).max(180),
      type: z.enum(["layout-concept", "figure-concept", "image-treatment", "supporting-visual", "diagram-rebuild"]),
      status: z.enum(["brief-ready", "concept-attached", "reconstruction-ready", "resolved", "held"]),
      sourceTextHash: z.string().regex(/^[0-9a-f]{64}$/),
      reason: z.string().min(1).max(1_000),
      communicationJob: z.string().min(1).max(1_000),
      expression: z.enum(["restrained", "balanced", "expressive"]),
      approvedInfluences: z.array(z.enum(["composition", "visual-hierarchy", "negative-space", "color-balance", "figure-concept", "image-treatment", "visual-rhythm"])).min(1).max(7),
      disclosurePolicy: z.enum(["abstract-structure-only", "exact-content-approved"]),
      approvedContentSummary: z.string().min(1).max(800).optional(),
      brandExpression: z.object({
        motif: z.enum(["pattern-free", "modular-square-grid", "directional-rule", "editorial-layering", "green-motion-gradient", "subordinate-hex-system"]),
        accent: z.enum(["none", "Energy", "Mist", "Biome", "Aqua", "Infinity", "Hydro", "Forge", "Spark", "Plasma", "Pulsar"]),
        accentRole: z.string().min(1).max(500),
        typographyStrategy: z.literal("no-generated-type-reserve-editable-aptos-zones"),
        rationale: z.string().min(1).max(1_000),
      }),
      structureInventory: z.object({ titleCount: z.number().int().nonnegative(), textGroupCount: z.number().int().nonnegative(), imageCount: z.number().int().nonnegative(), tableCount: z.number().int().nonnegative(), figureCount: z.number().int().nonnegative(), calloutCount: z.number().int().nonnegative() }),
      targetSlot: z.object({ role: z.enum(["whole-slide", "primary-visual", "supporting-evidence", "figure", "background-treatment"]), aspectRatio: z.enum(["16:9", "4:3", "1:1", "free"]), placementNotes: z.string().min(1).max(500) }),
      promptPackage: z.object({ prompt: z.string().min(1).max(4_000), negativePrompt: z.string().min(1).max(2_000), contentSafety: z.string().min(1).max(1_000) }),
      linkedConceptReferenceId: z.string().min(1).max(180).optional(),
      resolutionNote: z.string().min(1).max(1_000).optional(),
      createdAt: isoTimestamp,
      updatedAt: isoTimestamp,
    })).max(12).default([]),
    constraints: z.array(z.object({
      id: z.string().min(1).max(180), kind: z.enum(["align", "distribute", "snap-to-grid", "fit-safe-region"]), mode: z.enum(["left", "optical-left", "center", "right", "top", "optical-top", "middle", "bottom", "horizontal-equal-gap", "vertical-equal-gap", "both"]),
      nodeIds: z.array(z.string().min(1).max(180)).min(1).max(60), groups: z.array(z.array(z.string().min(1).max(180)).min(1).max(30)).max(30).optional(), anchorNodeId: z.string().min(1).max(180).optional(), gridPt: z.number().positive().max(72).optional(), rationale: z.string().min(1).max(1_000), author: z.enum(["human", "ai"]), evidenceAuthority: z.enum(["scene-estimate", "powerpoint-native"]), appliedAt: isoTimestamp,
    })).default([]),
    qualityReview: z.object({
      sceneRevision: z.string().min(1), slideUpdatedAt: isoTimestamp, rasterSha256: z.string().length(64), pass: z.number().int().min(1).max(3), maxPasses: z.literal(3),
      requestedVerdict: z.enum(["ready", "revise", "hold"]), recordedVerdict: z.enum(["ready", "revise", "hold"]), rationale: z.string().min(1).max(1_000), recordedAt: isoTimestamp,
      objectiveIssues: z.array(z.object({ id: z.string().min(1), category: z.enum(["overflow", "alignment", "spacing", "safe-region", "hierarchy", "figure", "brand", "legibility", "consistency", "other"]), severity: z.enum(["blocker", "major", "minor"]), source: z.enum(["powerpoint-native", "scene", "ai-visual"]), nodeIds: z.array(z.string()), message: z.string().min(1).max(1_000), recommendation: z.string().min(1).max(1_000), autoFixable: z.boolean() })).max(80),
      visualIssues: z.array(z.object({ id: z.string().min(1), category: z.enum(["overflow", "alignment", "spacing", "safe-region", "hierarchy", "figure", "brand", "legibility", "consistency", "other"]), severity: z.enum(["blocker", "major", "minor"]), source: z.literal("ai-visual"), nodeIds: z.array(z.string()), message: z.string().min(1).max(1_000), recommendation: z.string().min(1).max(1_000), autoFixable: z.boolean() })).max(40),
    }).optional(),
    nodes: z.array(studioNodeSchema), updatedAt: isoTimestamp,
  })),
});
const textBoxInventorySchema = z.object({
  id: z.string(),
  slideNumber: z.number().int().positive(),
  ordinal: z.number().int().positive(),
  shapeId: z.string(),
  text: z.string().default(""),
  textHash: z.string().regex(/^[0-9a-f]{64}$/),
  characterCount: z.number().int().nonnegative(),
  paragraphCount: z.number().int().nonnegative(),
  paragraphs: z.array(z.object({
    index: z.number().int().positive(),
    text: z.string(),
    textHash: z.string().regex(/^[0-9a-f]{64}$/),
    characterCount: z.number().int().nonnegative(),
    bullet: z.boolean(),
    bulletConfidence: z.enum(["direct", "inherited-possible"]),
    level: z.number().int().nonnegative(),
    fontFamilies: z.array(z.string()),
    fontSizes: z.array(z.number().nonnegative()),
  })).default([]),
  geometry: geometrySchema,
  textInsets: z.object({ left: z.number(), right: z.number(), top: z.number(), bottom: z.number() }).default({ left: 91_440, right: 91_440, top: 45_720, bottom: 45_720 }),
  paragraphLeftMarginsEmu: z.array(z.number()).default([]),
  paragraphIndentsEmu: z.array(z.number()).default([]),
  bulletParagraphCount: z.number().int().nonnegative().default(0),
  opticalLeftOffsetEmu: z.number().default(91_440),
  estimatedOpticalLeftEmu: z.number().default(0),
  opticalAlignmentConfidence: z.enum(["direct", "partial-inheritance"]).default("partial-inheritance"),
  fontFamilies: z.array(z.string()),
  fontSizes: z.array(z.number().nonnegative()),
  directFontSizeKnown: z.boolean(),
  paragraphAlignment: z.enum(["left", "center", "right", "justified", "mixed"]),
  verticalAlignment: z.enum(["top", "middle", "bottom"]),
  role: z.enum(["title", "body", "caption", "label", "other"]),
  autoFit: z.enum(["none", "shrink-text", "resize-shape", "unspecified"]),
  estimatedLineCount: z.number().int().positive(),
  estimatedRequiredHeightEmu: z.number().int().nonnegative(),
  fitRatio: z.number().nonnegative(),
  safeAreaStatus: z.enum(["inside", "near-edge", "off-slide"]),
  warnings: z.array(z.string()),
});
const layoutReviewSchema = z.object({
  id: z.string(),
  slideNumber: z.number().int().positive(),
  shapeId: z.string(),
  rule: z.enum(["overflow-risk", "off-slide", "safe-area", "alignment-ambiguous"]),
  severity: z.enum(["info", "warning", "error"]),
  confidence: z.enum(["high", "medium", "low"]),
  reason: z.string(),
  geometry: geometrySchema,
  fitRatio: z.number().nonnegative().optional(),
});
const alignmentRepairSchema = z.object({
  id: z.string(),
  slideNumber: z.number().int().positive(),
  shapeId: z.string(),
  textHash: z.string().regex(/^[0-9a-f]{64}$/),
  source: z.object({ x: z.number().int(), y: z.number().int(), width: z.number().int().nonnegative(), height: z.number().int().nonnegative() }),
  target: z.object({ x: z.number().int(), y: z.number().int(), width: z.number().int().nonnegative(), height: z.number().int().nonnegative() }),
  ruleId: z.enum(["cover.dominant-left-edge", "peer.dominant-left-edge"]),
  confidence: z.literal("high"),
  rationale: z.string(),
});
const findingSchema = z.object({
  id: z.string(),
  ruleId: z.string(),
  category: z.enum(["template", "font", "table", "figure", "layout", "production", "technical-review"]),
  severity: z.enum(["info", "warning", "error"]),
  confidence: z.enum(["high", "medium", "low"]),
  slideNumber: z.number().int().positive().optional(),
  message: z.string(),
  evidence: z.string(),
  autoFixable: z.boolean(),
});
const auditSchema = z.object({
  semanticVisualVersion: z.number().int().nonnegative().default(0),
  scannedAt: isoTimestamp,
  supportLevel: z.enum(["native-ooxml", "partial", "blocked"]),
  slideCount: z.number().int().nonnegative(),
  masterCount: z.number().int().nonnegative(),
  layoutCount: z.number().int().nonnegative(),
  themeCount: z.number().int().nonnegative(),
  notesCount: z.number().int().nonnegative(),
  legacyCommentCount: z.number().int().nonnegative(),
  modernCommentCount: z.number().int().nonnegative(),
  mediaCount: z.number().int().nonnegative(),
  tableCount: z.number().int().nonnegative(),
  chartCount: z.number().int().nonnegative(),
  pictureCount: z.number().int().nonnegative(),
  containsMacros: z.boolean(),
  containsOleObjects: z.boolean(),
  containsExternalRelationships: z.boolean(),
  externalHyperlinkCount: z.number().int().nonnegative().optional(),
  blockingExternalRelationshipCount: z.number().int().nonnegative().optional(),
  containsBlockingExternalRelationships: z.boolean().optional(),
  packageFileCount: z.number().int().positive(),
  expandedByteLength: z.number().int().nonnegative(),
  slideSize: z.object({ width: z.number().int().positive(), height: z.number().int().positive() }).default({ width: 12_192_000, height: 6_858_000 }),
  classification: z.enum(["current-ornl", "older-or-modified-ornl", "sponsor", "custom", "mixed", "unknown"]),
  classificationEvidence: z.array(z.string()),
  fonts: z.array(fontSchema),
  slides: z.array(slideSchema),
  tables: z.array(tableInventorySchema),
  pictures: z.array(pictureInventorySchema),
  textBoxes: z.array(textBoxInventorySchema).default([]),
  editableObjects: z.array(editableObjectSchema).default([]),
  layoutReviews: z.array(layoutReviewSchema).default([]),
  alignmentRepairs: z.array(alignmentRepairSchema).default([]),
  findings: z.array(findingSchema),
  warnings: z.array(z.string()),
}).transform((audit) => {
  const containsBlockingExternalRelationships = audit.containsBlockingExternalRelationships ?? audit.containsExternalRelationships;
  return {
    ...audit,
    externalHyperlinkCount: audit.externalHyperlinkCount ?? 0,
    blockingExternalRelationshipCount: audit.blockingExternalRelationshipCount ?? (containsBlockingExternalRelationships ? 1 : 0),
    containsBlockingExternalRelationships,
  };
});
const changeSchema = z.object({
  id: z.string(),
  kind: z.enum(["font-family", "table-style", "table-layout", "alignment", "geometry", "layout-remap", "text-style", "decoration"]),
  from: z.string(),
  to: z.string(),
  affectedSlideNumbers: z.array(z.number().int().positive()),
  affectedRunCount: z.number().int().nonnegative(),
  tableIds: z.array(z.string()).optional(),
  profileId: z.string().optional(),
  semanticColorPolicy: z.literal("preserve-source").optional(),
  tableLayoutCommands: z.array(z.object({
    id: z.string().min(1),
    slideNumber: z.number().int().positive(),
    tableId: z.string().min(1),
    objectId: z.string().min(1),
    columnWidthsEmu: z.array(z.number().int().positive()).min(1),
    rowHeightsEmu: z.array(z.number().int().positive()).min(1),
    cellMarginsEmu: z.object({ left: z.number().int().nonnegative(), right: z.number().int().nonnegative(), top: z.number().int().nonnegative(), bottom: z.number().int().nonnegative() }),
    rationale: z.string().min(1).max(700),
    author: z.enum(["human", "ai"]),
    constraints: z.object({ minimumFontPt: z.number().positive(), minimumHorizontalPaddingPt: z.number().nonnegative(), minimumVerticalPaddingPt: z.number().nonnegative(), preserveTableBounds: z.boolean() }),
    validation: z.object({ feasible: z.boolean(), predictedOverflowCellIds: z.array(z.string()), warnings: z.array(z.string()) }),
  })).optional(),
  alignmentRepairs: z.array(alignmentRepairSchema).optional(),
  geometryCommands: z.array(z.object({
    id: z.string(),
    slideNumber: z.number().int().positive(),
    objectId: z.string(),
    shapeId: z.string(),
    sourceElement: z.enum(["p:sp", "p:pic", "p:graphicFrame", "p:cxnSp", "p:grpSp"]),
    objectKind: z.enum(["text", "shape", "picture", "table", "chart", "connector", "group", "graphic-frame"]),
    operation: z.enum(["move", "resize", "move-and-resize"]),
    source: geometrySchema,
    target: geometrySchema,
    rationale: z.string(),
    author: z.enum(["human", "ai"]),
    constraints: z.object({
      allowIntentionalOverlap: z.boolean(),
      allowFitRisk: z.boolean(),
      allowSafeArea: z.boolean(),
      allowAspectRatioChange: z.boolean(),
    }).default({ allowIntentionalOverlap: false, allowFitRisk: false, allowSafeArea: false, allowAspectRatioChange: false }),
    validation: z.object({
      fitRatio: z.number().nonnegative().optional(),
      safeAreaStatus: z.enum(["inside", "near-edge"]),
      overlapObjectIds: z.array(z.string()),
      warnings: z.array(z.string()),
    }).default({ safeAreaStatus: "inside", overlapObjectIds: [], warnings: [] }),
  })).optional(),
  layoutCommands: z.array(z.object({
    id: z.string().min(1),
    slideNumber: z.number().int().positive(),
    templateSha256: z.string().length(64),
    templateLayoutPart: z.string().regex(/^ppt\/slideLayouts\/slideLayout\d+\.xml$/i),
    templateLayoutSha256: z.string().length(64),
    templateLayoutName: z.string().min(1),
    rationale: z.string().min(1).max(1_000),
    author: z.enum(["ai", "human"]),
  })).optional(),
  textStyleCommands: z.array(z.object({
    id: z.string().min(1),
    slideNumber: z.number().int().positive(),
    objectId: z.string().min(1),
    shapeId: z.string().min(1),
    typeface: z.literal("Aptos"),
    fontSizePt: z.number().min(10).max(60).optional(),
    bold: z.boolean().optional(),
    italic: z.boolean().optional(),
    color: z.string().regex(/^#[0-9A-F]{6}$/i).optional(),
    alignment: z.enum(["left", "center", "right"]).optional(),
    verticalAlignment: z.enum(["top", "middle", "bottom"]).optional(),
    insetsInches: z.object({ top: z.number().min(0).max(.25), right: z.number().min(0).max(.25), bottom: z.number().min(0).max(.25), left: z.number().min(0).max(.25) }).optional(),
    paragraphStyle: z.object({
      lineSpacingMultiple: z.number().min(.8).max(1.6).optional(),
      spaceAfterPt: z.number().min(0).max(30).optional(),
      bulletLeftMarginInches: z.number().min(0).max(1).optional(),
      bulletHangingInches: z.number().min(0).max(.5).optional(),
    }).optional(),
    rationale: z.string().min(1).max(700),
    author: z.enum(["ai", "human"]),
  })).optional(),
  decorationCommands: z.array(z.object({
    id: z.string().min(1),
    slideNumber: z.number().int().positive(),
    name: z.string().min(1).max(120),
    geometry: geometrySchema,
    fillColor: z.string().regex(/^#[0-9A-F]{6}$/i).optional(),
    lineColor: z.string().regex(/^#[0-9A-F]{6}$/i).optional(),
    lineWidthPt: z.number().min(0).max(6),
    behindContent: z.boolean(),
    rationale: z.string().min(1).max(700),
    author: z.enum(["ai", "human"]),
  })).optional(),
  rationale: z.string(),
  selected: z.boolean(),
});
const proposalSchema = z.object({
  id: z.string(),
  deckId: z.string(),
  baseUpdatedAt: isoTimestamp,
  createdAt: isoTimestamp,
  summary: z.string(),
  status: z.enum(["pending", "applied", "rejected"]),
  mode: z.enum(["font-cleanup", "designer-cleanup", "slide-geometry", "slide-reflow"]).default("font-cleanup"),
  standardVersion: z.string().optional(),
  designDecision: z.object({
    kind: z.literal("semantic-recomposition"),
    workOrderRevision: z.string().min(1),
    targetLayoutId: z.string().min(1),
    targetLayoutName: z.string().min(1),
    targetLayoutSourcePart: z.string().min(1),
    compatibilityScore: z.number().min(0).max(100),
    compatibilityStatus: z.enum(["recommended", "compatible", "poor", "incompatible"]),
    rationale: z.string().min(1).max(1_000),
    bindingCount: z.number().int().positive(),
    application: z.enum(["semantic-zones-on-source-layout", "cloned-native-layout"]),
  }).optional(),
  designReview: z.object({
    decision: z.literal("rejected"),
    actor: z.enum(["ai", "human"]),
    rationale: z.string().min(1).max(1_000),
    reviewedAt: isoTimestamp,
    evidence: z.object({
      slideNumber: z.number().int().positive(),
      renderer: z.literal("powerpoint-native"),
      currentRasterSha256: z.string().length(64),
      proposalRasterSha256: z.string().length(64),
      changedPixelRatio: z.number().min(0).max(1),
    }).optional(),
  }).optional(),
  visualIteration: z.object({
    maxAttempts: z.literal(3),
    history: z.array(z.object({
      attempt: z.number().int().min(1).max(3),
      slideNumber: z.number().int().positive(),
      inspectionRevision: z.string().min(1),
      verdict: z.enum(["better", "revise", "reject"]),
      rationale: z.string().min(1).max(1_000),
      reviewedAt: isoTimestamp,
      currentRasterSha256: z.string().length(64),
      proposalRasterSha256: z.string().length(64),
      metrics: z.object({ improvements: z.array(z.string()), regressions: z.array(z.string()) }),
      intentReview: z.object({
        status: z.enum(["pass", "needs-review"]),
        exactTextPreserved: z.boolean(),
        sourceVisualsPreserved: z.boolean(),
        relationshipsPreserved: z.enum(["yes", "not-applicable", "unverified"]),
        summary: z.string().min(1).max(1_000),
      }).default({ status: "needs-review", exactTextPreserved: true, sourceVisualsPreserved: true, relationshipsPreserved: "unverified", summary: "Legacy visual iteration predates original-intent review." }),
    })).max(3),
  }).optional(),
  changes: z.array(changeSchema),
  slideDispositions: z.array(z.object({
    slideNumber: z.number().int().positive(),
    status: z.enum(["change-proposed", "approved-as-is", "needs-review"]),
    reasons: z.array(z.string()),
    changeIds: z.array(z.string()),
  })).default([]),
  slideReviews: z.array(z.object({
    slideNumber: z.number().int().positive(),
    decision: z.enum(["approved", "changes-requested"]),
    reviewedAt: isoTimestamp,
    comment: z.string().min(1).max(4_000).optional(),
  })).default([]),
  tableExceptions: z.array(z.object({
    tableId: z.string(),
    slideNumber: z.number().int().positive(),
    reason: z.string(),
    rule: z.enum(["semantic-color", "complex-structure", "dense-table"]),
  })).default([]),
  layoutExceptions: z.array(layoutReviewSchema).default([]),
});
const deckSchema = z.object({
  id: z.string(),
  name: z.string(),
  sourceResourceId: z.string(),
  sourceSha256: z.string().regex(/^[0-9a-f]{64}$/),
  operationScope: z.enum(["audit-only", "cleanup-only", "reflow", "hybrid", "compose"]),
  templateClassification: z.enum(["current-ornl", "older-or-modified-ornl", "sponsor", "custom", "mixed", "unknown"]),
  targetTemplateId: z.string().optional(),
  targetTemplateConfirmedAt: isoTimestamp.optional(),
  targetTemplateDecisionSource: z.enum(["automatic-default", "automatic-source-preservation", "user-selected"]).optional(),
  designProfile: z.object({
    id: z.string().min(1),
    standardVersion: z.string().min(1),
    templateId: z.string().min(1),
    slideSize: z.literal("16:9"),
    fontFamily: z.literal("Aptos"),
    contentPolicy: z.literal("preserve-exact"),
    adoptedAt: isoTimestamp,
    source: z.enum(["automatic-default", "automatic-source-preservation", "user-selected"]),
    customized: z.boolean(),
  }).optional(),
  status: z.enum(["not-scanned", "audited", "needs-template-decision", "ready-for-cleanup", "proposal-ready", "needs-manual-review", "approved", "exported", "failed"]),
  audit: auditSchema.optional(),
  scene: z.preprocess((value) => value && typeof value === "object" && (value as { version?: unknown }).version !== PRESENTATION_SCENE_VERSION ? undefined : value, presentationSceneSchema.optional()),
  studioScene: z.preprocess((value) => value && typeof value === "object" && (value as { version?: unknown }).version !== STUDIO_WEB_SCENE_VERSION ? undefined : value, studioWebSceneSchema.optional()),
  proposal: proposalSchema.optional(),
  protectedSlideNumbers: z.array(z.number().int().positive()),
  nativeQualifiedStudioSlideRevisions: z.record(z.string(), isoTimestamp).default({}),
  failureMessage: z.string().max(700).optional(),
  exportedAt: isoTimestamp.optional(),
});

export const projectSchema = z.object({
  schema: z.literal(PROJECT_SCHEMA),
  schemaVersion: z.literal(PROJECT_SCHEMA_VERSION),
  project: z.object({
    id: z.string().min(1),
    name: z.string().min(1).max(240),
    type: z.enum(["review-batch", "single-deck", "new-presentation"]),
    createdAt: isoTimestamp,
    updatedAt: isoTimestamp,
  }),
  settings: z.object({
    contentPolicy: z.enum(["preserve-exact", "source-grounded-generative"]),
    defaultOperationScope: z.enum(["audit-only", "cleanup-only", "reflow", "hybrid", "compose"]),
    autosave: z.boolean(),
    designStandardVersion: z.string().min(1).default(defaultProjectDesignSettings().designStandardVersion),
    defaultProfileId: z.string().min(1).default(defaultProjectDesignSettings().defaultProfileId),
    defaultSlideSize: z.literal("16:9").default("16:9"),
    defaultFontFamily: z.literal("Aptos").default("Aptos"),
  }),
  resources: z.array(z.object({
    id: z.string().min(1),
    name: z.string().min(1),
    mediaType: z.string().min(1),
    byteLength: z.number().int().nonnegative(),
    sha256: z.string().regex(/^[0-9a-f]{64}$/),
    roles: z.array(z.enum(["import-origin", "prior-approved-revision", "style-exemplar", "concept-reference", "grounding-source", "slide-media", "chart-data", "template-source", "reference-only"])).min(1),
    kind: z.enum(["presentation", "document", "data", "image", "audio", "video", "other"]).optional(),
    support: z.array(z.enum(["source-readable", "previewable", "placeable", "pptx-preserved", "unsupported"])).optional(),
    processing: z.object({
      status: z.enum(["indexed", "stored-only", "needs-review"]),
      summary: z.string().max(1_000),
      processedAt: isoTimestamp,
      warnings: z.array(z.string().max(1_000)).max(50),
    }).optional(),
    derivatives: z.array(z.object({
      id: z.string().min(1),
      kind: z.literal("extracted-text"),
      mediaType: z.literal("text/plain"),
      byteLength: z.number().int().nonnegative(),
      sha256: z.string().regex(/^[0-9a-f]{64}$/),
      createdAt: isoTimestamp,
      processor: z.string().min(1).max(240),
      truncated: z.boolean(),
      bytes: z.instanceof(Uint8Array).optional(),
    })).max(20).optional(),
    createdAt: isoTimestamp,
    sourcePath: z.string().optional(),
    embedded: z.literal(true),
    bytes: z.instanceof(Uint8Array).optional(),
    mcpAccess: z.enum(["none", "metadata", "text", "preview"]),
  })),
  styleExemplars: z.array(z.object({
    id: z.string(),
    name: z.string(),
    kind: z.enum(["table", "figure"]),
    resourceId: z.string(),
    deckId: z.string(),
    slideNumber: z.number().int().positive(),
    objectOrdinal: z.number().int().positive(),
    scope: z.enum(["deck", "batch"]),
    createdAt: isoTimestamp,
  })),
  designThreads: z.array(z.object({
    id: z.string().min(1),
    deckId: z.string().min(1),
    slideId: z.string().min(1),
    slideNumber: z.number().int().positive(),
    outputSlideNumber: z.number().int().positive().optional(),
    baseRevision: isoTimestamp,
    anchor: z.object({
      kind: z.literal("region"),
      x: z.number().min(0).max(1),
      y: z.number().min(0).max(1),
      width: z.number().positive().max(1),
      height: z.number().positive().max(1),
    }),
    comment: z.string().min(1).max(4_000),
    status: z.enum(["note", "submitted", "proposal-ready", "resolved", "needs-reanchor"]),
    createdAt: isoTimestamp,
    updatedAt: isoTimestamp,
    submittedAt: isoTimestamp.optional(),
  })).default([]),
  decks: z.array(deckSchema),
  activity: z.array(z.object({ id: z.string(), at: isoTimestamp, action: z.string(), detail: z.string() })),
});

export function createProject(name = "Untitled review batch"): PresentationStudioProject {
  const now = new Date().toISOString();
  return {
    schema: PROJECT_SCHEMA,
    schemaVersion: PROJECT_SCHEMA_VERSION,
    project: {
      id: crypto.randomUUID(),
      name,
      type: "review-batch",
      createdAt: now,
      updatedAt: now,
    },
    settings: {
      contentPolicy: "preserve-exact",
      defaultOperationScope: "cleanup-only",
      autosave: true,
      ...defaultProjectDesignSettings(),
    },
    resources: [],
    styleExemplars: [],
    designThreads: [],
    decks: [],
    activity: [{ id: crypto.randomUUID(), at: now, action: "project-created", detail: "Created a conservative cleanup review batch." }],
  };
}

export function touchProject(project: PresentationStudioProject, action: string, detail: string): PresentationStudioProject {
  const now = new Date().toISOString();
  return {
    ...project,
    project: { ...project.project, updatedAt: now },
    activity: [...project.activity, { id: crypto.randomUUID(), at: now, action, detail }],
  };
}

export function projectForJson(project: PresentationStudioProject) {
  return {
    ...project,
    resources: project.resources.map(({ bytes: _bytes, sourcePath: _sourcePath, derivatives, ...resource }) => ({
      ...resource,
      mcpAccess: "none" as const,
      derivatives: derivatives?.map(({ bytes: _derivativeBytes, ...derivative }) => derivative),
    })),
  };
}
