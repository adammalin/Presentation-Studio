...
function metricIssues(slideNumber: number, metrics?: SlideDesignMetrics): DesignRepairIssue[] {
  ...
  if (metrics.textOverflowCount > 0) issues.push({ id: `slide-${slideNumber}-native-overflow`, slideNumber, category: "fit", severity: "error", status: "found", summary: `${metrics.textOverflowCount} PowerPoint-native text frame${metrics.textOverflowCount === 1 ? "" : "s"} overflow.`, evidence: "Rendered text exceeds its measured inner PowerPoint frame.", suggestedTreatment: "Choose a better shared layout or grow/reflow the measured frame; never silently shrink or rewrite approved text.", autoFixable: true });
