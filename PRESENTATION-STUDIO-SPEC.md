...
The first implemented vertical slice extracts source-bound text, tables, pictures, and preserved native objects into a canonical 13.333 × 7.5-inch Studio Web Scene while retaining a separate mapping to the imported slide size. Version 3 also records every directly readable source paragraph as a hash-bound semantic-atom candidate and reports whether the complete normalized slide text is mapped into Studio nodes. It exposes shared ORNL title/content, two-column, repeated comparison-card, table, figure-grid, footer, and installed-template recipes; provides drag, resize, and bounded type controls; persists the scene in `.pstudio`; and exposes it through MCP. The scene declares two compilers: `source-bound-overlay` maps web-computed geometry and typography back onto editable source objects, while `fresh-composition` builds a new editable PPTX from Aptos text, native tables, vectors, and extracted images. Both preserve exact visible-content and table-structure hashes on the qualified synthetic fixture. The fresh compiler also renders and measures its written artifact in Microsoft PowerPoint and rejects measured text overflow. It explicitly blocks preserved native internals and incompletely mapped grouped or inherited text rather than silently dropping them. Unsupported native objects remain visible in the editor through their cropped PowerPoint-native source pixels and stay locked until a safe whole-object operation or disclosed conversion is chosen.

Fresh composition must not inherit the source file's object granularity as a design limitation. A dense source text box may be split into several editable web and PowerPoint text objects, and grouped source content may be traversed into independently composable semantic atoms, only when all of the following are true: the complete source reading order is known; each atom retains an exact source locator and hash; the ordered normalized aggregate equals the approved source text; no wording, fact, label, note, or technical relationship is changed; and the native output passes the same exact-copy and visual-QA gates. Source-bound overlay remains one native object to one validated command and must not pretend that a paragraph fragment is independently editable in the imported package.

### 0.4 Specification 1.2 implementation increment: constraint and figure design loop

Specification 1.2 gives the AI and human editor stronger design operations without returning to raw PowerPoint coordinate guessing:

1. **Semantic layout constraints:** the Studio scene records align, optical-align, equal-gap distribute, grid, and safe-region constraints against stable node IDs or complete relationship groups. Text alignment can use the exact rendered text bounds measured by Microsoft PowerPoint rather than only the outer text-box rectangle. A constraint transaction is rejected when it would leave the canvas, deform a declared group, or introduce a new overlap.
2. **First-class technical figures:** related screenshots, labels, arrows, captions, and callouts can be declared as one figure treatment with a group frame, relationship map, focal point, crop window, aspect policy, and verification state. Source-locked figures move as one unit. A redraw candidate cannot replace its source until its complete information and relationship inventory is verified.
3. **Native-template-backed web design:** an approved Template Pack layout uses its PowerPoint-native full-slide render as the immutable visual base in the Studio canvas and fresh-composition export. Studio-authored content remains editable above it. This preserves brand imagery, geometry, and master-dependent details that a simplified HTML reconstruction cannot reproduce faithfully.
4. **One central current design:** Source is immutable evidence; the Studio Web Scene is the only editable design authority; Build compiles that exact scene; Slides, Studio, Review, and export consume the latest matching built revision. A changed slide invalidates only its own build and can be rebuilt incrementally before the deck is assembled and validated as a whole.
5. **Bounded native visual critic:** after Build, the AI receives the original PowerPoint-native pixels, the exact built pixels, semantic nodes, constraints, figure relationships, and deterministic findings. It runs at most three `Found issues -> Fixing -> Recheck` passes. Overflow, off-canvas content, broken optical alignment, unsafe figure crops, unreadable evidence, and unverified redraws block readiness; unresolved ambiguity is held for human review instead of looping.
6. **Deck design memory:** a slide that passes the native critic records a compact reusable design decision keyed to its exact content signature and Template Pack context. Later slides may reuse that recipe, hierarchy, grouping pattern, and rationale, but must still build and pass their own native visual critique.

Direct manipulation now includes multi-selection, snapping, center and peer guides, grid rhythm, complete-figure movement, and bounded crop/focal controls. Full undo/redo, direct relationship authoring, richer figure redrawing, reusable component editing, and native table editing remain later increments and must not be represented as complete.

### 0.5 Specification 1.3 implementation increment: Concept to Editable

Image generation may be used as an optional art-direction stage, never as the content or production-file authority. A generated or human-supplied raster enters the self-contained project as an immutable Image Resource. A person must separately authorize a bounded MCP preview before any connected model can see its pixels. The concept can then bind to one non-protected Studio slide with `concept-only` status, source-text hash, provenance, approved visual influences, a normalized semantic blueprint, and an explicit list of untrusted generated elements.

The source PowerPoint remains authoritative for wording, logos, marks, data, claims, scientific details, technical relationships, and approved media. A concept may influence composition, visual hierarchy, negative space, color balance, figure concept, image treatment, or visual rhythm only when those characteristics are named in the binding. Presentation Studio reconstructs them from semantic HTML/CSS components and exports editable PowerPoint objects; it does not trace the raster, copy generated typography, or flatten the concept into the final slide.

The required review compares three different authorities: Original vs. reconstruction for exact content and technical intent; Concept vs. reconstruction for the approved visual characteristics; and native PowerPoint reconstruction vs. the active ORNL Template Pack, accessibility, fit, editability, and production rules. Existing populated ORNL title slides cannot receive concept references.

### 0.6 Specification 1.4 implementation increment: visual-needs queue

Presentation Studio records the decision to seek visual art direction before any provider is involved. A visual need is stored on one non-protected slide with a source-text hash, communication job, need type, lifecycle state, structural inventory, target slot, ORNL expression recipe, disclosure boundary, and closed prompt package. The default boundary is `abstract-structure-only`; it never includes source wording or source pixels. `exact-content-approved` is available only when a person explicitly supplies a bounded approved summary.

The prompt package follows the proven ORNL Brand Agent control pattern: one primary raster, no rendered text, no logo, a closed allowed-visual inventory, a literal forbidden list, square geometry, one affirmative motif/pattern decision, and at most one purposeful secondary/accent role below 25%. It reserves clean zones for exact editable Aptos content rather than asking image generation to typeset the final slide. Studio does not call or privilege an image provider; any MCP-capable model may read the brief and use an authorized generator outside the app.

The lifecycle is `brief-ready → concept-attached → reconstruction-ready → resolved`, with `held` available at any point requiring human or content-owner direction. An imported concept remains an immutable Resource and needs explicit session Preview permission. Detaching or removing it reopens the brief without deleting the slide or the external original. A need resolves only when the editable reconstruction passes the bounded Microsoft PowerPoint-native visual review. Existing populated ORNL title slides cannot enter this workflow.

The active Brand Agent V2.3.2 prompt and 20-file Knowledge package may be mirrored into ignored local development storage for reference and regression design. Raw ORNL reference assets, unpublished prompt evidence, and official template bytes remain outside version control and are never treated as automatic upload material.

### 0.7 Specification 1.5 implementation increment: concept reconstruction and material impact

An attached concept becomes actionable only after an authorized model describes its approved composition as normalized semantic zones such as title, primary visual, supporting evidence, caption, and footer-safe space. `reconstruct_studio_concept` maps those zones through one shared Studio recipe to exact source-bound editable nodes. It never traces generated pixels, accepts generated wording or marks, or substitutes generated technical content for the source PowerPoint.

Studio records design impact as `unchanged`, `typography-only`, `cleanup`, `layout-redesign`, `figure-redesign`, or `full-redesign` by comparing source/current geometry, repeated-component assignments, visual-object placement, crop and figure treatments, and the active visual-need type. A requested layout concept cannot advance after a font-size adjustment; image and figure needs require material visual treatment; a diagram rebuild requires verified relationships and an editable reconstruction policy. The PowerPoint-native critic reports any unfulfilled brief as a major issue, so it cannot receive an AI-ready verdict.

Private regression uses a local hash-pinned manifest containing only absolute source/benchmark paths, SHA-256 identities, representative slide mappings, communication jobs, and review criteria. The runner writes source, candidate, and benchmark PowerPoint-native evidence outside Git, proves objective content/editability/overflow/impact gates, and leaves visual acceptance explicitly pending until each case is reviewed at full-slide size. Passing automated checks means “ready for visual review,” never “better by definition.”

### 0.8 Specification 1.6 implementation increment: responsive semantic recipes
...
Large-deck **Build all** is a visible background production job, not one blocking renderer/MCP transaction. The one persisted scene revision moves through preflight, editable compilation, approved ORNL Template Pack application, Microsoft PowerPoint rendering, Microsoft PowerPoint measurement, and hard production validation. The app remains usable, exposes the same stable job ID and phase to MCP, and permits cancellation of the exact private measurement bridge. A started, failed, canceled, or superseded job cannot become the Slides/export authority; only a `ready` job may replace the prior verified result. Native measurement retains a bounded long-deck lane because real 80–200-slide technical decks can require minutes to measure every text frame and editable table cell.
