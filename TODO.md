...
- [x] **Phase B — Canonical persisted Studio Web Scene.** Imported source-bound text, tables, images, and preserved objects compile into `presentation-studio/web-scene` version 2 with exact content, stable PowerPoint bindings, separate canonical and source slide sizes, semantic roles, source and computed frames, Aptos design tokens, media bindings, fidelity locks, and `.pstudio` persistence.
