...
- A hash-pinned, ignored private-golden harness qualifies seven representative BTO communication jobs against the immutable source and a human-cleaned design benchmark without committing customer content. Before rendering, it verifies that every declared benchmark slide actually represents the source slide's substantive content, preventing inserted/deleted continuation slides from silently producing a false ordinal comparison. Its objective gate writes a fresh editable PPTX, rerenders and remeasures it through Microsoft PowerPoint, and requires exact text/table structure, Aptos, zero true overflow, zero off-slide objects, native table-cell clearance, and material design impact before full-size visual review. Each run also emits a context-free MCP starter prompt so a fresh model can exercise the installed app without repository, benchmark, or parent-chat knowledge; source/candidate/golden pixels remain the acceptance evidence.
...
- The editable compiler returns an explicit rendered-node and rendered-figure-treatment coverage receipt to that critic. Missing figures, photographs, icons, logos, or source-locked evidence now block readiness even when PowerPoint itself reports clean geometry. Source-locked treatments replace—not accompany—their duplicate catalog representations; dense peer-logo walls cannot be flattened into a screenshot and use source-relative mark legibility; 19xx/20xx editorial records survive prior atomization and retain restrained year/title/neutral-attribution hierarchy at a bounded 10.5-point floor while image-bearing source-geometry editorial fields have a separately disclosed 8.5-point exception; and concise leftmost merged table context can repeat across merge-safe full-height ORNL continuation slides with a four-point native-clearance gutter.
...
# Prepare a benchmark-blind MCP acceptance packet for a genuinely fresh AI
# task. The emitted prompt names only the requested communication jobs, source
# hash, design-contract version, and a separate structured result path; it does
# not expose the private benchmark or its pixels.
npm run acceptance:black-box -- --qualification /tmp/presentation-studio-private-golden/private-golden-qualification.json --output /tmp/presentation-studio-black-box-run

# After the fresh task writes fresh-agent-result.json, validate and score it.
# An optional prior result establishes a pass-count trend across installed builds.
npm run acceptance:black-box -- --run /tmp/presentation-studio-black-box-run/black-box-agent-run.json --result /tmp/presentation-studio-black-box-run/fresh-agent-result.json --baseline /absolute/path/prior-fresh-agent-result.json --output /tmp/presentation-studio-black-box-run/evaluated
